								...
								<view v-for="(item, index) in tabItem.arr" :key="index" @click="cardClick(item)">
									<view v-if="item.cpFmtList && item.cpFmtList.length > 0" class="myCard2">
										...
										<view class="rightView">
											<view class="cardTopName1">{{item.name}}</view>
									...
									<view v-else class="myCard1">
										<view class="cardTopName">{{item.name}}</view>
										<view class="cardRow1">
											<view>产品价格：</view>
...
<script>
	...
	import { crmChanpinApi, crmCpClassifyApi, crmMyAppUserApi } from '@/utils/api.js'
	export default {
		components: {
			skeleton,
			getMore,
		...
		methods: {
			...
			getuserInfoFun: function(id) {
				...
				console.log('操作指令：'+reqData.action+' 传递参数：'+reqData.params)
