<template>
	<view class="content">
		...
		<z-paging ref="paging" :auto-clean-list-when-reload="false" :auto-scroll-to-top-when-reload="false"
			...
			<!--付款基本信息-->
			<payCard :item="khInfo" :isSelect="true" :djje="djje" :kjje="kjje" :F_Tax="F_Tax" :F_Total="F_Total" :itemList="tabList[0].arr"
				:index="cardIndex" :pagetype="'付款详情'" :product="tabList[0].arr"></payCard>
			<!--tab-->
			...
			<view class="contentView">
				...
				<view v-if="tabIndex == 0">
					<view v-for="(item, index) in tabList[0].arr" :key="index" @click="cardClick(item,index)">
						<payproduct :item="item" :index="selectIndex" searchLabel1="付款金额" searchPh1="请输入付款金额"
							searchLabel2="扣减金额" searchPh2="请输入扣减金额"></payproduct>
					</view>
				</view>
		...
		<u-popup v-model="searchShow" mode="center" width="666rpx" border-radius="14" :closeable="false">
			<view class="searchBox">
				...
				<view class="infoBox">
					<u-field v-model="khInfo.F_SupplierName" :label="'供方名称'" :placeholder="'请输入下单客户'" clear-size="40"
...
<script>
	...
	import payCard from '@/components/pay/pay.vue'
	import payproduct from '@/components/pay/payproduct.vue'
	...
	export default {
		...
		onLoad(options) {
			...
			if (that.cardIndex == -1) {
				...
				that.F_Date = this.$u.timeFormat(this.khInfo.F_Date, 'yyyy-mm-dd hh:MM:ss'),
					uni.$payInfo = that.khInfo;
				console.log('注意：是新增')
			} else {
				console.log('注意：是修改')
				that.khInfo = uni.$payInfo;
			...
			if (uni.$payInfo) {
				that.djje = uni.$payInfo.F_Money;
				that.kjje = uni.$payInfo.F_DisMoney;
				that.F_Tax = uni.$payInfo.F_Tax;
				that.F_Total = uni.$payInfo.F_Total;
			}
			...
			uni.$on('delpaydetailById', that.delpaydetailById)
			...
			uni.$on('refreshPayFun', that.refreshPayFun)
			//if (that.isLoadSelectKhById) {
			//	that.selectKhByIdFun();
			//}
			that.getPaydetailFun()
			that.getCurrencyFun();
			that.getSimpleFun();
			that.getYhzhFun();
			that.getXjllFun();			
		...
		onBackPress(e) {
			uni.$off('delpaydetailById', that.delpaydetailById)
			...
			uni.$off('refreshPayFun', that.refreshPayFun)
		...
		methods: {
			...
			pickerSelectFun: function(str) {
				...
				if (str == '对应客户') {
					let dykhid = that.dykhid;
					let arr = [];
					for (var i = 0; i < dykhid.length; i++) {
						arr.push({
							value: dykhid[i].F_ID,
							id: dykhid[i].F_ID,
							label: dykhid[i].F_Name
						})
					}
					that.selectList = arr;
				}
			...
			itemBind(e) {
				...
				this.kjje = e.total
				this.F_Tax = e.F_Tax
				this.F_Total = e.F_Total
				this.khInfo.F_Money = e.qty
				this.khInfo.F_DisMoney = e.total
				this.khInfo.F_Tax = e.F_Tax
				this.khInfo.F_Total = e.F_Total
				this.khInfo.rmb = e.F_Total * (this.khInfo.hl || 1)
			},
			selectClient(e) {

				this.khInfo.F_SupplierID = e.item.F_ID
				this.khInfo.F_SupplierName = e.item.name
				...
				this.khInfo.F_Bank1 = e.item.yhzh
				this.khInfo.F_Bank2 = e.item.yhzh1
				this.khInfo.F_Total = 0
				this.getDykhFun()
			},
			selectBank(e) {
			...
			delpaydetailById: function(e) {
				...
				for (var i = 0; i < arr.length; i++) {
					...
					F_Tax = +F_Tax + arr[i].F_Tax;
					F_Total = +F_Total + arr[i].F_Total;
				}
				that.djje = hjje;
				that.kjje = kjje;
				that.F_Tax = F_Tax;
				that.F_Total = F_Total;
			},
