<template>
	<view>
		...
		<dataNull v-if="list.length == 0" src="/static/img/chahua/dataNullXz.png" title="暂无相关付款单"
			title1="请更换查询条件"></dataNull>
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectPayFun"
			refresher-enabled :refresher-threshold="200" :refresher-triggered="triggered" refresher-background="gray"
			@refresherrefresh="onRefresh" @refresherrestore="onRestore">
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index" >
					<payCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'付款列表'"  @cxGetDataFun="cxGetDataFun"></payCard>
				</view>
		...
		<addBtn url="./payDetail?index=-1"></addBtn>
		
		<!--底部合计-->
		<view class="submitView">
			<view class="cardTopName disFlexJ">
				<text>合计：{{gs}}个单 付款￥: {{djje}}元 扣减: {{kjje}}元</text>
			</view>
		</view>
	</view>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.selectPayFun();
			uni.$on('addPayFun', that.addPayFun)
			uni.$on('deletePayFun', that.deletePayFun)
		...
		onBackPress() {
			uni.$off('addPayFun', that.addPayFun)
			uni.$off('deletePayFun', that.deletePayFun)
		...
		methods: {
			...
			deletePayFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addPayFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addPay);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addPay)
				let arr = that.list;
				arr.unshift(uni.$addPay);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
