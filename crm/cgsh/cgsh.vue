<template>
	<view>
		...
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectCgshFun"
			...
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="item.F_BillID" >
					<cgshCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'订单列表'" :priceRight="priceRight" :isCheck="item.sh"  @cxGetDataFun="cxGetDataFun"></cgshCard>
				</view>
		...
		<addBtn url="./cgshDetail?index=-1"></addBtn>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.cxGetDataFun();
			that.getRightFun();
			uni.$on('addCgshFun', that.addCgshFun)
			uni.$on('deleteCgshFun', that.deleteCgshFun)
		...
		onBackPress() {
			uni.$off('addCgshFun', that.addCgshFun)
			uni.$off('deleteCgshFun', that.deleteCgshFun)
		...
		methods: {
			...
			deleteCgshFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addCgshFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addCgsh);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addCgsh)
				let arr = that.list;
				arr.unshift(uni.$addCgsh);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
