<template>
	<view class="content">
		...
		<z-paging ref="paging" :auto-clean-list-when-reload="false" :auto-scroll-to-top-when-reload="false"
			...
			<!--订单基本信息-->
			<cgshCard :item="khInfo" :isSelect="true" :djje="djje" :itemList="tabList[0].arr" :index="cardIndex" :priceRight="priceRight"
				:pagetype="'订单详情'" :product="tabList[0].arr"></cgshCard>
			<!--tab-->
			...
			<view class="contentView">
				...
				<view v-if="tabIndex == 0">
					<view v-for="(item, index) in tabList[0].arr" :key="index" @click="cardClick(item,index)">
						<cgshproduct :item="item" :index="selectIndex" :priceRight="priceRight" searchLabel1="下单数量" searchPh1="请输入下单数量"
							searchLabel2="产品售价" searchPh2="请输入产品售价"></cgshproduct>
					</view>
				</view>
...
<script>
	...
	import cgshCard from '@/components/cgsh/cgsh.vue'
	import cgshproduct from '@/components/cgsh/cgshproduct.vue'
	...
	export default {
		...
		onLoad(options) {
			...
			console.log(uni.$cgshInfo)

			if (uni.$cgshInfo) {
				that.djje = uni.$cgshInfo.account;
			}
			...
			uni.$on('delcgshdetailById', that.delcgshdetailById)
			...
			uni.$on('refreshCgshFun', that.refreshCgshFun)
			
			that.getCgshdetailFun();
			that.getCurrencyFun();
			that.getSimpleFun();
			that.getCkmcFun();
			that.getPostcodeFun();
		...
		onBackPress(e) {
			uni.$off('delcgshdetailById', that.delcgshdetailById)
			...
			uni.$off('refreshCgshFun', that.refreshCgshFun)
