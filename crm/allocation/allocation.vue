<template>
	<view>
		...
		<dataNull v-if="list.length == 0" src="/static/img/chahua/dataNullXz.png" title="暂无相关调拨单"
			title1="请更换查询条件"></dataNull>
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectallocationFun"
			refresher-enabled :refresher-threshold="200" :refresher-triggered="triggered" refresher-background="gray"
			@refresherrefresh="onRefresh" @refresherrestore="onRestore">
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index" >
					<allocationCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'调拨列表'"  @cxGetDataFun="cxGetDataFun"></allocationCard>
				</view>
		...
		<addBtn url="./allocationDetail?index=-1"></addBtn>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.selectAllocationFun();
			uni.$on('addAllocationFun', that.addAllocationFun)
			uni.$on('deleteAllocationFun', that.deleteAllocationFun)
		...
		onBackPress() {
			uni.$off('addAllocationFun', that.addAllocationFun)
			uni.$off('deleteAllocationFun', that.deleteAllocationFun)
		...
		methods: {
			...
			deleteAllocationFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addAllocationFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addAllocation);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addAllocation)
				let arr = that.list;
				arr.unshift(uni.$addAllocation);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
