<template>
	<view class="content">
		...
		<z-paging ref="paging" :auto-clean-list-when-reload="false" :auto-scroll-to-top-when-reload="false"
			...
			<!--调拨单基本信息-->
			<allocationCard :item="khInfo" :isSelect="true" :djje="djje" :itemList="tabList[0].arr" :index="cardIndex"
				:pagetype="'调拨详情'" :product="tabList[0].arr"></allocationCard>
			<!--tab-->
			...
			<view class="contentView">
				...
				<view v-if="tabIndex == 0">
					<view v-for="(item, index) in tabList[0].arr" :key="index" @click="cardClick(item,index)">
						<allocationproduct :item="item" :index="selectIndex" searchLabel1="下单数量" searchPh1="请输入下单数量"
							searchLabel2="产品售价" searchPh2="请输入产品售价"></allocationproduct>
					</view>
				</view>
...
<script>
	...
	import allocationCard from '@/components/allocation/allocation.vue'
	import allocationproduct from '@/components/allocation/allocationproduct.vue'
	...
	export default {
		...
		onLoad(options) {
			...
			if (that.cardIndex == -1) {
				...
				that.khInfo = uni.$allocationInfo;
			}

			if (uni.$allocationInfo) {
				that.djje = uni.$allocationInfo.account;
			}
			...
			uni.$on('delallocationdetailById', that.delallocationdetailById)
			...
			uni.$on("selectCkmc2", this.selectCkmc2)
			uni.$on("selectPrintfile1", this.selectPrintfile)
			uni.$on('inputcountFun', that.inputcountFun)
			uni.$on("itemBind", that.itemBind)
			uni.$on('refreshAllocationFun', that.refreshAllocationFun)
			//if (that.isLoadSelectKhById) {
			//	that.selectKhByIdFun();
			//}
			that.getAllocationdetailFun()
		},
		onBackPress(e) {
			uni.$off('delallocationdetailById', that.delallocationdetailById)
		...
		methods: {
			// 通过下标更新list数据
			refreshAllocationFun: function(e) {
				that = this;
				that.khInfo = e.obj;
			...
			selectCkmc2(e) {
				...
				this.khInfo.ckmc1 = e.item.F_Name
			},
			selectPrintfile(e) {
			...
			delallocationdetailById: function(e) {
				var that = this;
				let hjje = 0;
			...
			// 获取调拨详情记录
			getAllocationdetailFun: function() {
				uni.showLoading({
					title: '加载中...',
					mask: true
				})
