<template>
	<view>
		...
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectSalesreturnFun"
...
<script>
	...
	export default {
		...
		onBackPress() {
			uni.$off('addSalesreturnFun', that.addSalesreturnFun)
			uni.$off('deleteSalesreturnFun', that.deleteSalesreturnFun)
		...
		methods: {
			...
			deleteSalesreturnFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addSalesreturnFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addSalesreturn);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addSalesreturn)
				let arr = that.list;
				arr.unshift(uni.$addSalesreturn);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
