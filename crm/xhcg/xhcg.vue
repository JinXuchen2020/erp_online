<template>
	<view>
		...
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectXhcgFun"
			...
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="item.F_BillID" >
					<xhcgCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'订单列表'" :isCheck="item.sh" :priceRight="priceRight"  @cxGetDataFun="cxGetDataFun"></xhcgCard>
				</view>
		...
		<addBtn url="./xhcgDetail?index=-1"></addBtn>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.cxGetDataFun();
			that.getRightFun();
			uni.$on('addXhcgFun', that.addXhcgFun)
			uni.$on('deleteXhcgFun', that.deleteXhcgFun)
		...
		onBackPress() {
			uni.$off('addXhcgFun', that.addXhcgFun)
			uni.$off('deleteXhcgFun', that.deleteXhcgFun)
		...
		methods: {
			...
			deleteXhcgFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addXhcgFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addXhcg);
				that.list = arr;
