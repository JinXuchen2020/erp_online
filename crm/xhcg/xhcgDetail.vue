...
<script>
	...
	import xhcgCard from '@/components/xhcg/xhcg.vue'
	import xhcgproduct from '@/components/xhcg/xhcgproduct.vue'
	...
	export default {
		...
		onLoad(options) {
			...
			console.log(uni.$xhcgInfo)

			if (uni.$xhcgInfo) {
				that.djje = uni.$xhcgInfo.account;
			}
			...
			uni.$on('delxhcgdetailById', that.delxhcgdetailById)
			...
			uni.$on('refreshXhcgFun', that.refreshXhcgFun)
			
			that.getXhcgdetailFun();
			that.getCurrencyFun();
			that.getSimpleFun();
			that.getCkmcFun();
			that.getPostcodeFun();
		...
		onBackPress(e) {
			...
			uni.$off('refreshXhcgFun', that.refreshXhcgFun)
