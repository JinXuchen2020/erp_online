<template>
	<view class="content">
		...
		<z-paging ref="paging" :auto-clean-list-when-reload="false" :auto-scroll-to-top-when-reload="false"
			...
			<!--清单单基本信息-->
			<bomCard :item="khInfo" :isSelect="true" :djje="djje" :itemList="tabList[0].arr" :index="cardIndex"
				:pagetype="'清单详情'" :product="tabList[0].arr"></bomCard>
			<!--tab-->
			...
			<view class="contentView">
				...
				<view v-if="tabIndex == 0">
					<view v-for="(item, index) in tabList[0].arr" :key="index" @click="cardClick(item,index)">
						<bomproduct :item="item" :index="selectIndex" searchLabel1="下单数量" searchPh1="请输入下单数量"
							searchLabel2="产品售价" searchPh2="请输入产品售价"></bomproduct>
					</view>
				</view>
...
<script>
	...
	import bomCard from '@/components/bom/bom.vue'	
	import bomproduct from '@/components/bom/bomproduct.vue'
	...
	export default {
		...
		onLoad(options) {
			...
			if (that.cardIndex == -1) {
				...
				that.khInfo = uni.$bomInfo;
			}

			if (uni.$bomInfo) {
				that.djje = uni.$bomInfo.account;
			}
			...
			uni.$on('delbomdetailById', that.delbomdetailById)
		...
		methods: {
			...
			selectPcode(e) {
				...
				this.khInfo.pcode = e.item.F_ID
				this.khInfo.pname = e.item.name
				this.khInfo.pspec = e.item.spec
				this.khInfo.pcolor = e.item.color
				this.khInfo.model = e.item.model
			},
			selectPrintfile(e) {
