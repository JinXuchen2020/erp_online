<template>
	<view>
		...
		<dataNull v-if="list.length == 0" src="/static/img/chahua/dataNullXz.png" title="暂无相关清单"
			title1="请更换查询条件"></dataNull>
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectbomFun"
			refresher-enabled :refresher-threshold="200" :refresher-triggered="triggered" refresher-background="gray"
			@refresherrefresh="onRefresh" @refresherrestore="onRestore">
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index" >
					<bomCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'清单列表'"  @cxGetDataFun="cxGetDataFun"></bomCard>
				</view>
		...
		<addBtn url="./bomDetail?index=-1"></addBtn>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.selectBomFun();
			uni.$on('addBomFun', that.addBomFun)
			uni.$on('deleteBomFun', that.deleteBomFun)
		...
		onBackPress() {
			uni.$off('addBomFun', that.addBomFun)
			uni.$off('deleteBomFun', that.deleteBomFun)
		...
		methods: {
			...
			deleteBomFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addBomFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addBom);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addBom)
				let arr = that.list;
				arr.unshift(uni.$addBom);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
