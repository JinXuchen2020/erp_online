<template>
	<view>
		...
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectSalesorderFun"
			...
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="item.F_BillID" >
					<salesorderCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'订单列表'" :isCheck="item.sh" :priceRight="priceRight" @cxGetDataFun="cxGetDataFun"></salesorderCard>
				</view>
		...
		<addBtn url="./salesorderDetail?index=-1"></addBtn>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.cxGetDataFun();
			that.getRightFun();
			uni.$on('addSalesorderFun', that.addSalesorderFun)
			uni.$on('deleteSalesorderFun', that.deleteSalesorderFun)
		...
		onBackPress() {
			uni.$off('addSalesorderFun', that.addSalesorderFun)
			uni.$off('deleteSalesorderFun', that.deleteSalesorderFun)
		...
		methods: {			
			...
			deleteSalesorderFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addSalesorderFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addSalesorder);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addSalesorder)
				let arr = that.list;
				arr.unshift(uni.$addSalesorder);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
