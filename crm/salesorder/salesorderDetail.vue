<template>
	<view class="content">
		...
		<z-paging ref="paging" :auto-clean-list-when-reload="false" :auto-scroll-to-top-when-reload="false"
			...
			<!--订单基本信息-->
			<salesorderCard :item="khInfo" :isSelect="true" :djje="djje" :itemList="tabList[0].arr" :index="cardIndex" :priceRight="priceRight"
				:pagetype="'订单详情'" :product="tabList[0].arr"></salesorderCard>
			<!--tab-->
			...
			<view class="contentView">
				...
				<view v-if="tabIndex == 0">
					<view v-for="(item, index) in tabList[0].arr" :key="index" @click="cardClick(item,index)">
						<salesorderproduct :item="item" :index="selectIndex" searchLabel1="下单数量" searchPh1="请输入下单数量" :priceRight="priceRight"
							searchLabel2="产品售价" searchPh2="请输入产品售价"></salesorderproduct>
					</view>
				</view>
		...
		<u-popup v-model="searchShow" mode="center" width="666rpx" border-radius="14" :closeable="false">
			<view class="searchBox">
				...
				<view class="infoBox">
					<u-field v-model="khInfo.clientname" :label="'客户名称'" :placeholder="'请输入下单客户'" clear-size="40"
				...
				<view @click="pickerSelectFun('贸易条款')" class="flex-white-plr26 ptb20 bdb_f5">
					<text class="mr26">贸易条款</text>
					<view  :class="khInfo.trade ? '' : 'cBlack'">
						{{khInfo.trade ? khInfo.trade : '请选择'}}
						<u-icon class="ml26" name="arrow-right" size="40" color="#888888"></u-icon>
					</view>
				</view>
...
<script>
	...
	import salesorderCard from '@/components/salesorder/salesorder.vue'
	import salesorderproduct from '@/components/salesorder/salesorderproduct.vue'
	...
	export default {
		...
		onLoad(options) {
			...
			if (that.cardIndex == -1) {
				...
				that.khInfo = uni.$salesorderInfo;
			...
			console.log(uni.$salesorderInfo)

			if (uni.$salesorderInfo) {
				that.djje = uni.$salesorderInfo.account;
			}
			...
			uni.$on('delsalesorderdetailById', that.delsalesorderdetailById)
			...
			uni.$on('refreshSalesorderFun', that.refreshSalesorderFun)
			
			that.getSalesorderdetailFun();
			that.getCurrencyFun();
			that.getSimpleFun();
			that.getTradeFun();
			that.getPostcodeFun();
		...
		onBackPress(e) {
			uni.$off('delsalesorderdetailById', that.delsalesorderdetailById)
			...
			uni.$off('refreshSalesorderFun', that.refreshSalesorderFun)
		...
		methods: {
			...
			selectConfirmFun: function(e) {
				...
				if (that.selectType == '贸易条款') {
					that.khInfo.trade = e[0].label;					
				}
			...
			// 通过下标更新list数据
			refreshSalesorderFun: function(e) {
				that = this;
				that.khInfo = e.obj;
			...
			delsalesorderdetailById: function(e) {
				var that = this;
				let hjje = 0;
			...
			// 获取订单详情记录
			getSalesorderdetailFun: function() {
				uni.showLoading({
					title: '加载中...',
					mask: true
				})
