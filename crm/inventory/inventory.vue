<template>
	<view>
		...
		<dataNull v-if="list.length == 0" src="/static/img/chahua/dataNullXz.png" title="暂无相关盘点单"
			title1="请更换查询条件"></dataNull>
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectinventoryFun"
			refresher-enabled :refresher-threshold="200" :refresher-triggered="triggered" refresher-background="gray"
			@refresherrefresh="onRefresh" @refresherrestore="onRestore">
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index" >
					<inventoryCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'盘点列表'"  @cxGetDataFun="cxGetDataFun"></inventoryCard>
				</view>
		...
		<addBtn url="./inventoryDetail?index=-1"></addBtn>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.selectInventoryFun();
			uni.$on('addInventoryFun', that.addInventoryFun)
			uni.$on('deleteInventoryFun', that.deleteInventoryFun)
		...
		onBackPress() {
			uni.$off('addInventoryFun', that.addInventoryFun)
			uni.$off('deleteInventoryFun', that.deleteInventoryFun)
		...
		methods: {
			...
			deleteInventoryFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addInventoryFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addInventory);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addInventory)
				let arr = that.list;
				arr.unshift(uni.$addInventory);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
