	...
	<view>
				...
				<view class="myCard">
					...
					<view class="myCard3">
						...
						<view class="infoBox">
							...
							<view class="cardRow">
								<view>现有库存：</view>
								<view><text class="redColor">{{item.kcsl}}</text></view>
							</view>							
							<view class="cardRow" v-if="item.F_ID">
								<view>产品编码：</view>
								<view>{{item.F_ID}}</view>
							</view>
		...
		<uni-popup ref="popucountt" mode="center" width="666rpx" border-radius="14" :closeable="false">
			<view class="searchBox">
				...
				<u-field v-model="countt" :label="'盘点数量'" :placeholder="'请输入盘点数量'" clear-size="40"></u-field>
				<!-- <view class="searchTitle">输入单价</view> -->
				<u-field v-model="kcsl" :label="'现有库存'" :placeholder="'请输入现有库存'" clear-size="40"></u-field>
	...
	export default {
		...
		methods: {
			...
			setsum() {
				...
				for (var i = 0; i < this.itemList.length; i++) {
					if (this.itemList[i].countt) {
						...
						this.total += +this.itemList[i].countt - +this.itemList[i].kcsl
					}
				}
			},
			delcountt(index) {
			...
			selectItem(index) {
				...
				this.currency = index
				this.kcsl = this.list[index].kcsl ? this.list[index].kcsl : 0
			...
			inputcountFun: function() {
				...
				if(num>=0){
					...
					this.itemList[num].pksl=+this.countt- +this.kcsl
				}
				else{
					var item={}
					...
					item.kcsl=this.list[this.currency].kcsl
					item.pksl=(+this.list[this.currency].countt)- (+this.list[this.currency].kcsl)
