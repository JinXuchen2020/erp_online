<template>
	<view>
		...
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectclientdreportFun"
			...
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index" @click="khCardClickFun(item)">
					<clientdreportCard :item="item" :isSelect="isSelect" :index="index" @cxGetDataFun="cxGetDataFun"></clientdreportCard>
				</view>
...
<script>
	...
	import clientdreportCard from '@/components/card/clientdreport.vue'
	export default {
		components: {
			dataNull,
			getMore,			
		...
		methods: {
			...
			// 查询客户
			selectclientdreportFun: function() {
				if (!that.isMore) {
					return
				}
				uni.showLoading({
			...
			khCardClickFun: function(item) {
				...
				if (that.pageType == 'lxr') {
					...
					uni.$khInfo = item;
					uni.$emit('clientdreportBindFun', item)
					uni.navigateBack()
				}
			...
			addItemInListFun: function() {
				...
				let arr = that.list;
				arr.unshift(uni.$addClientdreport);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
