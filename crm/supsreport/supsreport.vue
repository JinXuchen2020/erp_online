<template>
	<view>
		...
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectsupsreportFun"
			...
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index" @click="khCardClickFun(item)">
					<supsreportCard :item="item" :isSelect="isSelect" :index="index" @cxGetDataFun="cxGetDataFun"></supsreportCard>
				</view>
		...
		methods: {
			...
			khCardClickFun: function(item) {
				...
				if (that.pageType == 'lxr') {
					...
					uni.$khInfo = item;
					uni.$emit('supsreportBindFun', item)
					uni.navigateBack()
				}
