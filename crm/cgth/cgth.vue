<template>
	<view>
		...
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectCgthFun"
			...
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="item.F_BillID" >
					<cgthCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'订单列表'" :isCheck="item.sh" :priceRight="priceRight"  @cxGetDataFun="cxGetDataFun"></cgthCard>
				</view>
		...
		<addBtn url="./cgthDetail?index=-1"></addBtn>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.cxGetDataFun();
			that.getRightFun();
			uni.$on('addCgthFun', that.addCgthFun)
			uni.$on('deleteCgthFun', that.deleteCgthFun)
		...
		onBackPress() {
			uni.$off('addCgthFun', that.addCgthFun)
			uni.$off('deleteCgthFun', that.deleteCgthFun)
		...
		methods: {
			...
			deleteCgthFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addCgthFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addCgth);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addCgth)
				let arr = that.list;
				arr.unshift(uni.$addCgth);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
