<template>
	<view class="content">
		...
		<z-paging ref="paging" :auto-clean-list-when-reload="false" :auto-scroll-to-top-when-reload="false"
			...
			<!--订单基本信息-->
			<cgthCard :item="khInfo" :isSelect="true" :djje="djje" :itemList="tabList[0].arr" :index="cardIndex" :priceRight="priceRight"
				:pagetype="'订单详情'" :product="tabList[0].arr"></cgthCard>
			<!--tab-->
			...
			<view class="contentView">
				...
				<view v-if="tabIndex == 0">
					<view v-for="(item, index) in tabList[0].arr" :key="index" @click="cardClick(item,index)">
						<cgthproduct :item="item" :index="selectIndex" :priceRight="priceRight" searchLabel1="下单数量" searchPh1="请输入下单数量"
							searchLabel2="产品售价" searchPh2="请输入产品售价"></cgthproduct>
					</view>
				</view>
		...
		<u-popup v-model="searchShow" mode="center" width="666rpx" border-radius="14" :closeable="false">
			<view class="searchBox">
				...
				<view @click="pickerSelectFun('业务员')" class="flex-white-plr26 ptb20 bdb_f5">
					<text class="mr26">QA跟进</text>
					<view  :class="khInfo.QA ? '' : 'cBlack'">
						{{khInfo.QA ? khInfo.QA : '请选择'}}
						<u-icon class="ml26" name="arrow-right" size="40" color="#888888"></u-icon>
					</view>
				</view>
				...
				<u-field v-model="khInfo.bz" :label="'收货备注'" :placeholder="'请输入收货备注'" clear-size="40"></u-field>
...
<script>
	...
	import cgthCard from '@/components/cgth/cgth.vue'
	import cgthproduct from '@/components/cgth/cgthproduct.vue'
	...
	export default {
		...
		onLoad(options) {
			...
			console.log(uni.$cgthInfo)

			if (uni.$cgthInfo) {
				that.djje = uni.$cgthInfo.account;
			}
			...
			uni.$on('delcgthdetailById', that.delcgthdetailById)
			...
			uni.$on('refreshCgthFun', that.refreshCgthFun)
			
			that.getCgthdetailFun();
			that.getCurrencyFun();
			that.getSimpleFun();
			that.getCkmcFun();
			that.getPostcodeFun();
		...
		onBackPress(e) {
			uni.$off('delcgthdetailById', that.delcgthdetailById)
			...
			uni.$off('refreshCgthFun', that.refreshCgthFun)
