	...
	<view>
		...
		<uni-popup ref="popucountt" mode="center" width="666rpx" border-radius="14" :closeable="false">
			<view class="searchBox">
				...
				<u-field v-model="cgsl" :label="'收货数量'" :placeholder="'请输入收货数量'" clear-size="40"></u-field>
				<u-field v-model="F_FPTaxrate" :label="'发票税率'" :placeholder="'请输入发票税率'" clear-size="40"></u-field>
				
				<u-field v-model="price" :label="'收货单价'" :placeholder="'请输入收货单价'" clear-size="40"></u-field>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.dykh = e.dykh;
			that.clientcode = e.clientcode;
			console.log('dykh:' + that.dykh + ' clientcode:' + that.clientcode)
			setTimeout(() => {
				that.getChanPinFun();
			}, 200)
		...
		methods: {
			...
			delcountt(index) {
				// console.log(index);
				this.list[index].cgsl = 0	
				this.list[index].F_FPTaxrate = 0
				var num = this.itemList.findIndex(v => ((v.itemcode === this.list[index].itemcode) && (v.dyxs === this
					.list[index].F_BillID)))
			...
			selectItem(index) {
				...
				this.F_FPTaxrate = this.list[index].F_FPTaxrate
				this.currency = index
			...
			inputcountFun: function() {
				...
				this.list[this.currency].cgsl = +this.cgsl
				this.list[this.currency].F_FPTaxrate = +this.F_FPTaxrate	
				this.list[this.currency].price = +this.price
				this.list[this.currency].check = true
				var num = this.itemList.findIndex(v => ((v.itemcode === this.list[this.currency].itemcode) && (v
					.dyxs === this.list[this.currency].F_BillID)))
				console.log("a", num);
				if (num >= 0) {
					this.itemList[num].cgsl = +this.cgsl
					this.itemList[num].F_FPTaxrate = +this.F_FPTaxrate
					this.itemList[num].price = +this.price
					this.itemList[num].account = +this.cgsl * +this.price
					this.itemList[num].F_FPTotal = (+this.cgsl * +this.price)* +this.F_FPTaxrate /100
				} else {
					var item = {}
					item.Aid = this.itemList.length
					item.itemcode = this.list[this.currency].itemcode
					item.itemname = this.list[this.currency].itemname
					item.spec = this.list[this.currency].spec
					item.color = this.list[this.currency].color
					item.unit = this.list[this.currency].unit
					item.countt = this.list[this.currency].cgsl					
					item.price = this.list[this.currency].price
					item.account = +this.list[this.currency].cgsl * +this.list[this.currency].price
					item.bz = this.list[this.currency].bz
					item.url = this.list[this.currency].url
					item.dyxs = this.list[this.currency].dyxs
					item.xsddid = this.list[this.currency].F_BillID
					item.PO = this.list[this.currency].PO
					item.orderdate=this.list[this.currency].enddate
					item.xsid=this.list[this.currency].Aid
					item.F_TypeName=this.list[this.currency].F_TypeName
					item.F_Taxrate=this.list[this.currency].F_Taxrate
					item.F_Tax = +this.list[this.currency].cgsl * +this.list[this.currency].price * +this.list[this.currency].F_Taxrate /100
					item.F_Total = (+this.list[this.currency].cgsl * +this.list[this.currency].price * +this.list[this.currency].F_Taxrate /100) +(+this.list[this.currency].cgsl * +this.list[this.currency].price )
					item.F_FPTaxrate = this.list[this.currency].F_FPTaxrate
					
					item.F_FPTotal = (+this.list[this.currency].cgsl * +this.list[this.currency].price)* +this.list[this.currency].F_FPTaxrate /100
					
					item.cgsl=this.list[this.currency].countt
					...
					.then(res => {
						...
						for (var i = 0; i < data.length; i++) {
							for (var j = 0; j < itemList.length; j++) {
								if ((itemList[j].itemcode == data[i].itemcode) && (itemList[j].dyxs == data[i]
										.F_BillID)) {
									data[i].cgsl = itemList[j].countt
									data[i].price = itemList[j].price
									data[i].F_FPTaxrate = itemList[j].F_FPTaxrate
