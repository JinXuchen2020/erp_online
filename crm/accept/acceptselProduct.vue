	...
	<view>
		...
		<scroll-view v-else scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="getChanPinFun"
			...
			<view v-for="(item, index) in list" :key="index" @click="cardClick(item)">
				<view class="myCard">
					<view class="cardTopName">销售单号：{{item.F_BillID}}</view>
					<view class="myCard3">
		...
		<uni-popup ref="popucountt" mode="center" width="666rpx" border-radius="14" :closeable="false">
			<view class="searchBox">
				...
				<u-field v-model="F_Money" :label="'收款金额'" :placeholder="'请输入收款金额'" clear-size="40"></u-field>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.itemList = uni.getStorageSync("acceptdetail")
		...
		methods: {
			...
			selectItem(index) {
				console.log(index);
				console.log('获得的金额',this.list[index].F_TotalMoney);
				if(this.F_Money=='')
				{
					this.F_Money= this.list[index].F_TotalMoney
				}
				else
				{
			...
			inputcountFun: function() {
				...
				console.log('this.list的F_BillID',this.list[this.currency].F_BillID)
				var num = this.itemList.findIndex(v=>v.F_LinkBillID === this.list[this.currency].F_BillID)
				console.log("a",num);
				if(num>=0){
					this.itemList[num].F_Money=+this.F_Money
					this.itemList[num].F_DisMoney=+this.F_DisMoney
					console.log('找到了原来的itemlist',num)
				}
				else{
					var item={}
					...
					console.log('新增itemlist',num)
				}
				this.$refs.popucountt.close()
				if(this.itemList.length)
				{
					console.log('this.itemList的记录大于0',this.itemList[0].F_ThisMoney);
					this.setsum()					
					this.F_Money = ""
					this.F_DisMoney = ""
				}
				...
				{
					console.log('this.itemList的记录为空');
					this.setsum()					
					this.F_Money = ""
					this.F_DisMoney = ""
				}
