<template>
	<view>
		...
		<dataNull v-if="list.length == 0" src="/static/img/chahua/dataNullXz.png" title="暂无相关收款单"
			title1="请更换查询条件"></dataNull>
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectAcceptFun"
			refresher-enabled :refresher-threshold="200" :refresher-triggered="triggered" refresher-background="gray"
			@refresherrefresh="onRefresh" @refresherrestore="onRestore">
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index" >
					<acceptCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'收款列表'"  @cxGetDataFun="cxGetDataFun"></acceptCard>
				</view>
		...
		<addBtn url="./acceptDetail?index=-1"></addBtn>
		
		<!--底部合计-->
		<view class="submitView">
			<view class="cardTopName disFlexJ">
				<text>合计：{{gs}}个单 收款￥: {{djje}}元 扣减: {{kjje}}元</text>
			</view>
		</view>
	</view>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.selectAcceptFun();
			uni.$on('addAcceptFun', that.addAcceptFun)
			uni.$on('deleteAcceptFun', that.deleteAcceptFun)
		...
		onBackPress() {
			uni.$off('addAcceptFun', that.addAcceptFun)
			uni.$off('deleteAcceptFun', that.deleteAcceptFun)
		...
		methods: {
			...
			deleteAcceptFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addAcceptFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addAccept);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addAccept)
				let arr = that.list;
				arr.unshift(uni.$addAccept);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
