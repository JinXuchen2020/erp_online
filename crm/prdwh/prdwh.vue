<template>
	<view>
		...
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectprdwhFun"
			refresher-enabled :refresher-threshold="200" :refresher-triggered="triggered" refresher-background="gray"
			@refresherrefresh="onRefresh" @refresherrestore="onRestore">
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index" >
					<prdwhCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'入库列表'"  @cxGetDataFun="cxGetDataFun"></prdwhCard>
				</view>
		...
		<addBtn url="./prdwhDetail?index=-1"></addBtn>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.selectPrdwhFun();
			uni.$on('addPrdwhFun', that.addPrdwhFun)
			uni.$on('deletePrdwhFun', that.deletePrdwhFun)
		...
		onBackPress() {
			uni.$off('addPrdwhFun', that.addPrdwhFun)
			uni.$off('deletePrdwhFun', that.deletePrdwhFun)
		...
		methods: {
			...
			deletePrdwhFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addPrdwhFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addPrdwh);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addPrdwh)
				let arr = that.list;
				arr.unshift(uni.$addPrdwh);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
