<template>
	<view class="content">
		...
		<z-paging ref="paging" :auto-clean-list-when-reload="false" :auto-scroll-to-top-when-reload="false"
			...
			<!--入库单基本信息-->
			<prdwhCard :item="khInfo" :isSelect="true" :djje="djje" :itemList="tabList[0].arr" :index="cardIndex"
				:pagetype="'入库详情'" :product="tabList[0].arr"></prdwhCard>
			<!--tab-->
			...
			<view class="contentView">
				...
				<view v-if="tabIndex == 0">
					<view v-for="(item, index) in tabList[0].arr" :key="index" @click="cardClick(item,index)">
						<prdwhproduct :item="item" :index="selectIndex" searchLabel1="下单数量" searchPh1="请输入下单数量"
							searchLabel2="产品售价" searchPh2="请输入产品售价"></prdwhproduct>
					</view>
				</view>
...
<script>
	...
	import prdwhCard from '@/components/prdwh/prdwh.vue'
	import prdwhproduct from '@/components/prdwh/prdwhproduct.vue'
	...
	export default {
		...
		onLoad(options) {
			...
			if (that.cardIndex == -1) {
				...
				that.khInfo = uni.$prdwhInfo;
			}

			if (uni.$prdwhInfo) {
				that.djje = uni.$prdwhInfo.account;
			}
			...
			uni.$on('delprdwhdetailById', that.delprdwhdetailById)
			...
			uni.$on('refreshPrdwhFun', that.refreshPrdwhFun)
			//if (that.isLoadSelectKhById) {
			//	that.selectKhByIdFun();
			//}
			that.getPrdwhdetailFun()
		},
		onBackPress(e) {
			uni.$off('delprdwhdetailById', that.delprdwhdetailById)
			...
			uni.$off('refreshPrdwhFun', that.refreshPrdwhFun)
