<template>
	<view class="content">
		...
		<z-paging ref="paging" :auto-clean-list-when-reload="false" :auto-scroll-to-top-when-reload="false"
			...
			<!--客户基本信息-->
			<payableCard :item="khInfo" :index="cardIndex" :isSelect="true"></payableCard>
			...
			<view class="contentView">
				...
				<view v-if="tabIndex == 0">
					<view v-for="(item, index) in tabList[0].arr" :key="index">
						<xsfh :item="item"></xsfh>
					</view>
				</view>
				<!--销售退货-->
				<view v-if="tabIndex == 1">
					<view v-for="(item, index) in tabList[1].arr" :key="index">
						<xsth :item="item" :isHideMore="true"></xsth>
					</view>
				</view>
				<!--销售收款-->
...
<script>
	...
	import payableCard from '@/components/card/payable.vue'
	import xsfh from '@/components/card/xsfh.vue'
	import xsth from '@/components/card/xsth.vue'
	import xssk from '@/components/card/xssk.vue'	
	...
	export default {
		...
		onLoad(e) {
			...
			uni.$on('refreshPayableFun', that.refreshPayableFun)
		},
		onBackPress(e) {
			...
			uni.$off('refreshPayableFun', that.refreshPayableFun)
		},
		onShow() {
