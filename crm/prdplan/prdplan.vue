<template>
	<view>
		...
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectprdplanFun"
			refresher-enabled :refresher-threshold="200" :refresher-triggered="triggered" refresher-background="gray"
			@refresherrefresh="onRefresh" @refresherrestore="onRestore">
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index" >
					<prdplanCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'计划列表'"  @cxGetDataFun="cxGetDataFun"></prdplanCard>
				</view>
		...
		<addBtn url="./prdplanDetail?index=-1"></addBtn>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.selectPrdplanFun();
			uni.$on('addPrdplanFun', that.addPrdplanFun)
			uni.$on('deletePrdplanFun', that.deletePrdplanFun)
		...
		onBackPress() {
			uni.$off('addPrdplanFun', that.addPrdplanFun)
			uni.$off('deletePrdplanFun', that.deletePrdplanFun)
		...
		methods: {
			...
			deletePrdplanFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addPrdplanFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addPrdplan);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addPrdplan)
				let arr = that.list;
				arr.unshift(uni.$addPrdplan);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
