<template>
	<view class="content">
		...
		<z-paging ref="paging" :auto-clean-list-when-reload="false" :auto-scroll-to-top-when-reload="false"
			...
			<!--计划单基本信息-->
			<prdplanCard :item="khInfo" :isSelect="true" :djje="djje" :itemList="tabList[0].arr" :index="cardIndex"
				:pagetype="'计划详情'" :product="tabList[0].arr"></prdplanCard>
			<!--tab-->
			...
			<view class="contentView">
				...
				<view v-if="tabIndex == 0">
					<view v-for="(item, index) in tabList[0].arr" :key="index" @click="cardClick(item,index)">
						<prdplanproduct :item="item" :index="selectIndex" searchLabel1="下单数量" searchPh1="请输入下单数量"
							searchLabel2="产品售价" searchPh2="请输入产品售价"></prdplanproduct>
					</view>
				</view>
...
<script>
	...
	import prdplanCard from '@/components/prdplan/prdplan.vue'	
	import prdplanproduct from '@/components/prdplan/prdplanproduct.vue'
	...
	export default {
		...
		onLoad(options) {
			...
			if (that.cardIndex == -1) {
				...
				that.khInfo = uni.$prdplanInfo;
			}

			if (uni.$prdplanInfo) {
				that.djje = uni.$prdplanInfo.account;
			}
			...
			uni.$on('delprdplandetailById', that.delprdplandetailById)
			...
			uni.$on('refreshPrdplanFun', that.refreshPrdplanFun)
			//if (that.isLoadSelectKhById) {
			//	that.selectKhByIdFun();
			//}
			that.getPrdplandetailFun()
		},
		onBackPress(e) {
			uni.$off('delprdplandetailById', that.delprdplandetailById)
			...
			uni.$off('refreshPrdplanFun', that.refreshPrdplanFun)
		...
		methods: {
			...
			delprdplandetailById: function(e) {
				var that = this;
				let hjje = 0;
