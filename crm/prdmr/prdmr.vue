<template>
	<view>
		...
		<dataNull v-if="list.length == 0" src="/static/img/chahua/dataNullXz.png" title="暂无相关领料单"
			title1="请更换查询条件"></dataNull>
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectprdmrFun"
			refresher-enabled :refresher-threshold="200" :refresher-triggered="triggered" refresher-background="gray"
			@refresherrefresh="onRefresh" @refresherrestore="onRestore">
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index" >
					<prdmrCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'领料列表'"  @cxGetDataFun="cxGetDataFun"></prdmrCard>
				</view>
		...
		<addBtn url="./prdmrDetail?index=-1"></addBtn>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.selectPrdmrFun();
			uni.$on('addPrdmrFun', that.addPrdmrFun)
			uni.$on('deletePrdmrFun', that.deletePrdmrFun)
		...
		onBackPress() {
			uni.$off('addPrdmrFun', that.addPrdmrFun)
			uni.$off('deletePrdmrFun', that.deletePrdmrFun)
		...
		methods: {
			...
			deletePrdmrFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addPrdmrFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addPrdmr);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addPrdmr)
				let arr = that.list;
				arr.unshift(uni.$addPrdmr);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
