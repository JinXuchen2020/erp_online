<template>
	<view class="content">
		...
		<z-paging ref="paging" :auto-clean-list-when-reload="false" :auto-scroll-to-top-when-reload="false"
			...
			<!--领料单基本信息-->
			<prdmrCard :item="khInfo" :isSelect="true" :djje="djje" :itemList="tabList[0].arr" :index="cardIndex"
				:pagetype="'领料详情'" :product="tabList[0].arr"></prdmrCard>
			<!--tab-->
			...
			<view class="contentView">
				...
				<view v-if="tabIndex == 0">
					<view v-for="(item, index) in tabList[0].arr" :key="index" @click="cardClick(item,index)">
						<prdmrproduct :item="item" :index="selectIndex" searchLabel1="下单数量" searchPh1="请输入下单数量"
							searchLabel2="产品售价" searchPh2="请输入产品售价"></prdmrproduct>
					</view>
				</view>
...
<script>
	...
	import prdmrCard from '@/components/prdmr/prdmr.vue'	
	import prdmrproduct from '@/components/prdmr/prdmrproduct.vue'
	...
	export default {
		...
		onLoad(options) {
			...
			if (that.cardIndex == -1) {
				...
				that.khInfo = uni.$prdmrInfo;
			}

			if (uni.$prdmrInfo) {
				that.djje = uni.$prdmrInfo.account;
			}
			...
			uni.$on('delprdmrdetailById', that.delprdmrdetailById)
			...
			uni.$on('refreshPrdmrFun', that.refreshPrdmrFun)
			//if (that.isLoadSelectKhById) {
			//	that.selectKhByIdFun();
			//}
			that.getPrdmrdetailFun()
		},
		onBackPress(e) {
			uni.$off('delprdmrdetailById', that.delprdmrdetailById)
			...
			uni.$off('refreshPrdmrFun', that.refreshPrdmrFun)
		...
		methods: {
			...
			itemBind(e) {
				...
				for (var i = 0; i < this.tabList[0].arr.length; i++) {
					if (this.tabList[0].arr[i].rksl) {
						this.tabList[0].arr[i].countt=this.tabList[0].arr[i].rksl
						this.tabList[0].arr[i].account=this.tabList[0].arr[i].rksl * +this.tabList[0].arr[i].price
					}
				}
