<template>
	<view>
		...
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectprdsreportFun"
			...
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index" @click="khCardClickFun(item)">
					<prdsreportCard :item="item" :isSelect="isSelect" :index="index" @cxGetDataFun="cxGetDataFun"></prdsreportCard>
				</view>
...
<script>
	...
	import prdsreportCard from '@/components/card/prdsreport.vue'
	export default {
		components: {
			dataNull,
			getMore,			
		...
		methods: {
			...
			khCardClickFun: function(item) {
				...
				if (that.pageType == 'lxr') {
					...
					uni.$khInfo = item;
					uni.$emit('prdsreportBindFun', item)
					uni.navigateBack()
				}
