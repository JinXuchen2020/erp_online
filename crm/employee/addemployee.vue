	...
	export default {
		...
		onLoad(e) {
			...
			that.pageType=e.pageType||'';	
				...
				{
					console.log('得到的客户编号：'+uni.$addEmployee.F_ID)
					that.F_ID= uni.$addEmployee.F_ID;
					that.name= uni.$addEmployee.name|| '';
					that.type=uni.$addEmployee.type|| '';
					that.Ename= uni.$addEmployee.Ename|| '';
					that.contactman= uni.$addEmployee.contactman|| '';
					that.tel= uni.$addEmployee.tel|| '';
					that.city= uni.$addEmployee.city|| '';
					that.fax= uni.$addEmployee.fax|| '';
					that.currency= uni.$addEmployee.currency|| '';
					that.simple= uni.$addEmployee.simple|| '';
					that.trade= uni.$addEmployee.trade|| '';
					that.postcode= uni.$addEmployee.postcode|| '';
					that.address= uni.$addEmployee.address|| '';
					that.packing= uni.$addEmployee.packing|| '';
					that.packingE= uni.$addEmployee.packingE|| '';
					that.wxmw= uni.$addEmployee.wxmw|| '';
					that.wxmwE= uni.$addEmployee.wxmwE|| '';
					that.standard= uni.$addEmployee.standard|| '';
					that.standardE= uni.$addEmployee.standardE|| '';
					that.yxsx= uni.$addEmployee.yxsx|| '';
					that.yxsm= uni.$addEmployee.yxsm|| '';
					that.PayDay= uni.$addEmployee.PayDay|| '';
					that.ed= uni.$addEmployee.ed|| '';
					that.fzr= uni.$addEmployee.fzr|| '';
					that.bz= uni.$addEmployee.bz|| '';
					that.F_Type= uni.$addEmployee.F_Type|| '';						
				...
				.then(res => {
					...
					if(code=='0'){
						...
						if(this.isAdd==1)
						{
							...
							uni.$addEmployee = res.data.rows[0];
							// uni.$emit('addItemInListFun')
						}
						
						
						uni.$emit('refreshEmployeeFun')
