<template>
	<view class="content">
		...
		<z-paging ref="paging" :auto-clean-list-when-reload="false" :auto-scroll-to-top-when-reload="false"
			...
			<!--委外单基本信息-->
			<subcontractCard :item="wwxdInfo" :isSelect="true" :djje="djje" :itemList="tabList[0].arr" :index="cardIndex"
				:pagetype="'委外详情'" :product="tabList[0].arr"></subcontractCard>
			<!--tab-->
			...
			<view class="contentView">
				...
				<view v-if="tabIndex == 0">
					<view v-for="(item, index) in tabList[0].arr" :key="index" @click="cardClick(item,index)">
						<subcontractproduct :item="item" :index="selectIndex" searchLabel1="下单数量" searchPh1="请输入下单数量"
							searchLabel2="产品售价" searchPh2="请输入产品售价"></subcontractproduct>
					</view>
				</view>
...
<script>
	...
	import subcontractCard from '@/components/subcontract/subcontract.vue'	
	import subcontractproduct from '@/components/subcontract/subcontractproduct.vue'
	...
	export default {
		...
		onLoad(options) {
			...
			if (that.cardIndex == -1) {
				...
				that.wwxdInfo = uni.$subcontractInfo;
			}

			if (uni.$subcontractInfo) {
				that.djje = uni.$subcontractInfo.account;
			}
			...
			uni.$on('delsubcontractdetailById', that.delsubcontractdetailById)
			...
			uni.$on("selectBm1", this.selectBm)
			uni.$on("selectPrintfile1", this.selectPrintfile)
			uni.$on('inputcountFun', that.inputcountFun)
			uni.$on("itemBind", that.itemBind)
			uni.$on('refreshSubcontractFun', that.refreshSubcontractFun)
			//if (that.isLoadSelectKhById) {
			//	that.selectKhByIdFun();
			//}
			that.getSubcontractdetailFun()
		},
		onBackPress(e) {
			uni.$off('delsubcontractdetailById', that.delsubcontractdetailById)
		...
		methods: {
			// 通过下标更新list数据
			refreshSubcontractFun: function(e) {
				that = this;
				that.wwxdInfo = e.obj;
			...
			itemBind(e) {
				...
				this.djje = e.total
				this.wwxdInfo.account = e.total

			},
			selectSupplier(e) {
				this.wwxdInfo.clientcode = e.item.F_ID
				this.wwxdInfo.clientname = e.item.name
			},
			selectPayment(e) {
				this.wwxdInfo.fkfs = e.item.F_Name
			},
			selectBm(e) {
				this.wwxdInfo.clientcode = e.item.F_ID
				this.wwxdInfo.clientname = e.item.name
			...
			selectPrintfile(e) {
				...
				{
					let params = {
						F_BillID: this.wwxdInfo.F_BillID,
			...
			delsubcontractdetailById: function(e) {
				var that = this;
				let hjje = 0;
			...
			// 获取委外详情记录
			getSubcontractdetailFun: function() {
				uni.showLoading({
					title: '加载中...',
					mask: true
				})
				...
				let params = {
					...
					F_BillID: that.wwxdInfo.F_BillID,
					pageIndex: item.pageIndex,
					pageSize: 20
				}
				let reqData = {
					action: 'selectSubcontractdetail',
					params: JSON.stringify(params)
				}
