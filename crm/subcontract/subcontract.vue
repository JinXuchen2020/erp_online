<template>
	<view>
		...
		<dataNull v-if="list.length == 0" src="/static/img/chahua/dataNullXz.png" title="暂无相关委外单"
			title1="请更换查询条件"></dataNull>
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectsubcontractFun"
			refresher-enabled :refresher-threshold="200" :refresher-triggered="triggered" refresher-background="gray"
			@refresherrefresh="onRefresh" @refresherrestore="onRestore">
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index" >
					<subcontractCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'委外列表'"  @cxGetDataFun="cxGetDataFun"></subcontractCard>
				</view>
		...
		<addBtn url="./subcontractDetail?index=-1"></addBtn>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.selectSubcontractFun();
			uni.$on('addSubcontractFun', that.addSubcontractFun)
			uni.$on('deleteSubcontractFun', that.deleteSubcontractFun)
		...
		onBackPress() {
			uni.$off('addSubcontractFun', that.addSubcontractFun)
			uni.$off('deleteSubcontractFun', that.deleteSubcontractFun)
		...
		methods: {
			...
			deleteSubcontractFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addSubcontractFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addSubcontract);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addSubcontract)
				let arr = that.list;
				arr.unshift(uni.$addSubcontract);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
