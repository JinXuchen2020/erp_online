<template>
	<view>
		...
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectreceivablesFun"
			...
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index" @click="khCardClickFun(item)">
					<receivablesCard :item="item" :isSelect="isSelect" :index="index" @cxGetDataFun="cxGetDataFun"></receivablesCard>
				</view>
...
<script>
	...
	import receivablesCard from '../../components/card/receivables.vue'
	export default {
		components: {
			dataNull,
			getMore,			
		...
		methods: {
			...
			selectreceivablesFun: function() {
				...
				let reqObj = {
					...
					model:'frmAcceptAccount'
				}
				let reqData = {
					action: 'selectReceivables',
			...
			khCardClickFun: function(item) {
				...
				if (that.pageType == 'lxr') {
					...
					uni.$khInfo = item;
					uni.$emit('receivablesBindFun', item)
					uni.navigateBack()
				}
			...
			addItemInListFun: function() {
				...
				let arr = that.list;
				arr.unshift(uni.$addReceivables);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
