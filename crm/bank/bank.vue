<template>
	<view>
		...
		<dataNull v-if="list.length == 0" src="/static/img/chahua/dataNullXz.png" title="暂无相关账户"
			title1="请更换查询条件"></dataNull>
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectbankFun"
			...
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index" @click="khCardClickFun(item)">
					<bankCard :item="item" :isSelect="isSelect" :index="index" @cxGetDataFun="cxGetDataFun"></bankCard>
				</view>
		...
		<addBtn url="./addbank?isAdd=1&&pageType=''"></addBtn>
...
<script>
	...
	import bankCard from '@/components/card/bank.vue'
	export default {
		components: {
			dataNull,
			addBtn,
		...
		onLoad(e) {
			...
			that.selectbankFun();
			uni.$on('deleteBankFun', that.deleteBankFun)
		...
		onBackPress() {
			uni.$off('deleteBankFun', that.deleteBankFun)
