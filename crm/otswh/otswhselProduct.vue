<template>
	<view>
				...
				<view class="myCard">
					...
					<view class="myCard3">
						...
						<view class="infoBox">
							<view class="cardRow">
								<view>产品编号：</view>
								<view><text class="redColor">{{item.itemcode}} </text> </view>
							</view>
							<view class="cardRow">
								<view>产品规格：</view>
					...
					<view class="rowBtn">
						...
						<view class="rkslcss" v-if="item.rksl">
							入库：{{item.rksl}}
						</view>
						<u-button type="error" :plain="true" class="cpBtn" size="mini" @click="delrksl(index)">
							删除</u-button>
					</view>
					<!--搜索弹窗-->
		...
		<uni-popup ref="popurksl" mode="center" width="666rpx" border-radius="14" :closeable="false">
			<view class="searchBox">
				<!-- <view class="searchTitle">输入下单数量</view> -->
				<u-field v-model="rksl" :label="'入库数量'" :placeholder="'请输入入库数量'" clear-size="40"></u-field>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			that = this;
			that.clientcode=e.ckmc;
			console.log('ckmc',e.ckmc)
			uni.getSystemInfo({
				success(res) {
		...
		methods: {
			...
			setsum() {
				...
				for (var i = 0; i < this.itemList.length; i++) {
					if (this.itemList[i].rksl) {
						this.qty += +this.itemList[i].rksl
						this.total += +this.itemList[i].rksl* +this.itemList[i].price
					}
				}
			},
			delrksl(index) {
				// console.log(index);
				this.list[index].rksl = 0
				var num = this.itemList.findIndex(v => v.f_guid === this.list[index].f_guid)
			...
			popuclose() {
				this.$refs.popurksl.close()
			},
			selectItem(index) {
				console.log(index);
				this.rksl = this.list[index].rksl
				this.currency = index
				this.price = this.list[index].price ? this.list[index].price : 0
				this.$refs.popurksl.open()
			},
			// 搜索处理函数
			inputcountFun: function() {
				if(+this.rksl>(+this.list[this.currency].count2))
				{
					uni.showModal({
						title: '提示',
						content: '进仓数量太多，请核查',
				...
				else
				{
					...
					this.list[this.currency].rksl = +this.rksl
					this.list[this.currency].price = +this.price
					this.list[this.currency].check = true
					var num = this.itemList.findIndex(v => v.f_guid === this.list[this.currency].f_guid)
					console.log("a", num);
					if (num >= 0) {
						this.itemList[num].rksl = +this.rksl
						this.itemList[num].price = +this.price
						this.itemList[num].account = +this.rksl*  +this.price
					} else {
						var item = {}
						...
						item.rksl = this.list[this.currency].rksl
						item.price = this.list[this.currency].price
						item.account = (+this.list[this.currency].rksl) * (+this.list[this.currency].price)
