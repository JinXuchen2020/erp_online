<template>
	<view>
		...
		<dataNull v-if="list.length == 0" src="/static/img/chahua/dataNullXz.png" title="暂无相关进仓单"
			title1="请更换查询条件"></dataNull>
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectotswhFun"
			refresher-enabled :refresher-threshold="200" :refresher-triggered="triggered" refresher-background="gray"
			@refresherrefresh="onRefresh" @refresherrestore="onRestore">
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index" >
					<otswhCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'进仓列表'"  @cxGetDataFun="cxGetDataFun"></otswhCard>
				</view>
		...
		<addBtn url="./otswhDetail?index=-1"></addBtn>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.selectOtswhFun();
			uni.$on('addOtswhFun', that.addOtswhFun)
			uni.$on('deleteOtswhFun', that.deleteOtswhFun)
		...
		onBackPress() {
			uni.$off('addOtswhFun', that.addOtswhFun)
			uni.$off('deleteOtswhFun', that.deleteOtswhFun)
		...
		methods: {
			...
			deleteOtswhFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addOtswhFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addOtswh);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addOtswh)
				let arr = that.list;
				arr.unshift(uni.$addOtswh);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
