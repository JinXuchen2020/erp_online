<template>
	<view class="content">
		...
		<z-paging ref="paging" :auto-clean-list-when-reload="false" :auto-scroll-to-top-when-reload="false"
			...
			<!--进仓单基本信息-->
			<otswhCard :item="khInfo" :isSelect="true" :djje="djje" :itemList="tabList[0].arr" :index="cardIndex"
				:pagetype="'进仓详情'" :product="tabList[0].arr"></otswhCard>
			<!--tab-->
			...
			<view class="contentView">
				...
				<view v-if="tabIndex == 0">
					<view v-for="(item, index) in tabList[0].arr" :key="index" @click="cardClick(item,index)">
						<otswhproduct :item="item" :index="selectIndex" searchLabel1="下单数量" searchPh1="请输入下单数量"
							searchLabel2="产品售价" searchPh2="请输入产品售价"></otswhproduct>
					</view>
				</view>
...
<script>
	...
	import otswhCard from '@/components/otswh/otswh.vue'
	import otswhproduct from '@/components/otswh/otswhproduct.vue'
	...
	export default {
		...
		onLoad(options) {
			...
			if (that.cardIndex == -1) {
				...
				that.khInfo = uni.$otswhInfo;
			}

			if (uni.$otswhInfo) {
				that.djje = uni.$otswhInfo.account;
			}
			...
			uni.$on('delotswhdetailById', that.delotswhdetailById)
			...
			uni.$on('refreshOtswhFun', that.refreshOtswhFun)
			//if (that.isLoadSelectKhById) {
			//	that.selectKhByIdFun();
			//}
			that.getOtswhdetailFun()
		},
		onBackPress(e) {
			uni.$off('delotswhdetailById', that.delotswhdetailById)
			...
			uni.$off('refreshOtswhFun', that.refreshOtswhFun)
