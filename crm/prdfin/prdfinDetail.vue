<template>
	<view class="content">
		...
		<z-paging ref="paging" :auto-clean-list-when-reload="false" :auto-scroll-to-top-when-reload="false"
			...
			<!--计划单基本信息-->
			<prdfinCard :item="khInfo" :isSelect="true" :djje="djje" :itemList="tabList[0].arr" :index="cardIndex"
				:pagetype="'计划详情'" :product="tabList[0].arr"></prdfinCard>
			<!--tab-->
			...
			<view class="contentView">
				...
				<view v-if="tabIndex == 0">
					<view v-for="(item, index) in tabList[0].arr" :key="index" @click="cardClick(item,index)">
						<prdfinproduct :item="item" :index="selectIndex" searchLabel1="下单数量" searchPh1="请输入下单数量"
							searchLabel2="产品售价" searchPh2="请输入产品售价"></prdfinproduct>
					</view>
				</view>
...
<script>
	...
	import prdfinCard from '@/components/prdfin/prdfin.vue'	
	import prdfinproduct from '@/components/prdfin/prdfinproduct.vue'
	...
	export default {
		...
		onLoad(options) {
			...
			if (that.cardIndex == -1) {
				...
				that.khInfo = uni.$prdfinInfo;
			}

			if (uni.$prdfinInfo) {
				that.djje = uni.$prdfinInfo.account;
			}
			...
			uni.$on('delprdfindetailById', that.delprdfindetailById)
			...
			uni.$on('refreshPrdfinFun', that.refreshPrdfinFun)
			//if (that.isLoadSelectKhById) {
			//	that.selectKhByIdFun();
			//}
			that.getPrdfindetailFun()
		},
		onBackPress(e) {
			uni.$off('delprdfindetailById', that.delprdfindetailById)
			...
			uni.$off('refreshPrdfinFun', that.refreshPrdfinFun)
		...
		methods: {
			...
			delprdfindetailById: function(e) {
				var that = this;
				let hjje = 0;
