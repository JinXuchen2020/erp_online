<template>
	<view>
		...
		<dataNull v-if="list.length == 0" src="/static/img/chahua/dataNullXz.png" title="暂无相关计划单"
			title1="请更换查询条件"></dataNull>
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectprdfinFun"
			refresher-enabled :refresher-threshold="200" :refresher-triggered="triggered" refresher-background="gray"
			@refresherrefresh="onRefresh" @refresherrestore="onRestore">
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index" >
					<prdfinCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'计划列表'"  @cxGetDataFun="cxGetDataFun"></prdfinCard>
				</view>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.selectPrdfinFun();
			uni.$on('addPrdfinFun', that.addPrdfinFun)
			uni.$on('deletePrdfinFun', that.deletePrdfinFun)
		...
		onBackPress() {
			uni.$off('addPrdfinFun', that.addPrdfinFun)
			uni.$off('deletePrdfinFun', that.deletePrdfinFun)
		...
		methods: {
			...
			deletePrdfinFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addPrdfinFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addPrdfin);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addPrdfin)
				let arr = that.list;
				arr.unshift(uni.$addPrdfin);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
