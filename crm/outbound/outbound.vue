<template>
	<view>
		...
		<dataNull v-if="list.length == 0" src="/static/img/chahua/dataNullXz.png" title="暂无相关出库单"
			title1="请更换查询条件"></dataNull>
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectoutboundFun"
			refresher-enabled :refresher-threshold="200" :refresher-triggered="triggered" refresher-background="gray"
			@refresherrefresh="onRefresh" @refresherrestore="onRestore">
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index" >
					<outboundCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'出库列表'"  @cxGetDataFun="cxGetDataFun"></outboundCard>
				</view>
		...
		<addBtn url="./outboundDetail?index=-1"></addBtn>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.selectOutboundFun();
			uni.$on('addOutboundFun', that.addOutboundFun)
			uni.$on('deleteOutboundFun', that.deleteOutboundFun)
		...
		onBackPress() {
			uni.$off('addOutboundFun', that.addOutboundFun)
			uni.$off('deleteOutboundFun', that.deleteOutboundFun)
		...
		methods: {
			...
			deleteOutboundFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addOutboundFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addOutbound);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addOutbound)
				let arr = that.list;
				arr.unshift(uni.$addOutbound);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
