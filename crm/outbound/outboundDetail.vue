<template>
	<view class="content">
		...
		<z-paging ref="paging" :auto-clean-list-when-reload="false" :auto-scroll-to-top-when-reload="false"
			...
			<!--出库单基本信息-->
			<outboundCard :item="khInfo" :isSelect="true" :djje="djje" :itemList="tabList[0].arr" :index="cardIndex"
				:pagetype="'出库详情'" :product="tabList[0].arr"></outboundCard>
			<!--tab-->
			...
			<view class="contentView">
				...
				<view v-if="tabIndex == 0">
					<view v-for="(item, index) in tabList[0].arr" :key="index" @click="cardClick(item,index)">
						<outboundproduct :item="item" :index="selectIndex" searchLabel1="下单数量" searchPh1="请输入下单数量"
							searchLabel2="产品售价" searchPh2="请输入产品售价"></outboundproduct>
					</view>
				</view>
...
<script>
	...
	import outboundCard from '@/components/outbound/outbound.vue'	
	import outboundproduct from '@/components/outbound/outboundproduct.vue'
	...
	export default {
		...
		onLoad(options) {
			...
			if (that.cardIndex == -1) {
				...
				that.khInfo = uni.$outboundInfo;
			}

			if (uni.$outboundInfo) {
				that.djje = uni.$outboundInfo.account;
			}
			...
			uni.$on('deloutbounddetailById', that.deloutbounddetailById)
			...
			uni.$on('refreshOutboundFun', that.refreshOutboundFun)
			//if (that.isLoadSelectKhById) {
			//	that.selectKhByIdFun();
			//}
			that.getOutbounddetailFun()
		},
		onBackPress(e) {
			uni.$off('deloutbounddetailById', that.deloutbounddetailById)
			...
			uni.$off('refreshOutboundFun', that.refreshOutboundFun)
		...
		methods: {
			...
			deloutbounddetailById: function(e) {
				var that = this;
				let hjje = 0;
