<template>
	<view>
		...
		<dataNull v-if="list.length == 0" src="/static/img/chahua/dataNullXz.png" title="暂无相关发货单"
			title1="请更换查询条件"></dataNull>
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectDelivergdsFun"
...
<script>
	...
	export default {
		...
		onBackPress() {
			uni.$off('addDelivergdsFun', that.addDelivergdsFun)
			uni.$off('deleteDelivergdsFun', that.deleteDelivergdsFun)
		...
		methods: {
			...
			deleteDelivergdsFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addDelivergdsFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addDelivergds);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addDelivergds)
				let arr = that.list;
				arr.unshift(uni.$addDelivergds);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
