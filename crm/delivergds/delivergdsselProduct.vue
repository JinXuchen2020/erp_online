	...
	<view>
		...
		<scroll-view v-else scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="getChanPinFun"
			...
			<view v-for="(item, index) in list" :key="index" @click="cardClick(item)">
				<view class="myCard">
					<view class="cardTopName">产品名称：{{item.itemname}}</view>
					<view class="myCard3">
						...
						<view class="infoBox">
							<view class="cardRow">
								<view>订货单号：</view>
								<view><text class="redColor">{{item.F_BillID}} </text> </view>
							</view>
							<view class="cardRow">
								<view>订货日期：</view>
							...
							<view class="cardRow">
								<view>欠货数量：</view>
								<view><text class="redColor">{{item.count2}}</text> </view>
							</view>
							<view class="cardRow">
								<view>产品价格：</view>
							...
							<view class="cardRow" v-if="item.ckmc">
								<view>所属仓库：</view>
								<view>{{item.ckmc}} </view>
							</view>
							<view class="cardRow" v-if="item.stock">
								<view>产品库存：</view>
								<view><text class="redColor">{{item.stock}}</text>/{{item.unit}} </view>
							</view>
					...
					<view class="rowBtn">
						...
						<view class="fhslcss" v-if="item.fhsl">
							发货数量：
							{{item.fhsl}}
						</view>
						<u-button type="error" :plain="true" class="cpBtn" size="mini" @click="delfhsl(index)">
							删除</u-button>
					</view>
					<!--搜索弹窗-->
		...
		<uni-popup ref="popufhsl" mode="center" width="666rpx" border-radius="14" :closeable="false">
			<view class="searchBox">
				<!-- <view class="searchTitle">输入下单数量</view> -->
				<u-field v-model="fhsl" :label="'发货数量'" :placeholder="'请输入发货数量'" clear-size="40"></u-field>
		...
		methods: {
			...
			setsum() {
				...
				for (var i = 0; i < this.itemList.length; i++) {
					if (this.itemList[i].fhsl) {
						this.qty += +this.itemList[i].fhsl
						this.total += +this.itemList[i].fhsl * +this.itemList[i].price
					}
				}
			},
			delfhsl(index) {
				// console.log(index);
				this.list[index].fhsl = 0
			...
			popuclose() {
				this.$refs.popufhsl.close()
			},
			selectItem(index) {
				this.fhsl = this.list[index].fhsl
				this.currency = index
				this.price = this.list[index].price ? this.list[index].price : 0
				this.$refs.popufhsl.open()
			},
			// 搜索处理函数
			inputcountFun: function() {
				if (this.fhsl > this.list[this.currency].stock) {
					uni.showModal({
						title: '提示',
						content: '发货数量大于库存数量',
						showCancel: false
				...
				else
				{
					...
					this.list[this.currency].fhsl = +this.fhsl
					this.list[this.currency].price = +this.price
					this.list[this.currency].check = true
					var num = this.itemList.findIndex(v => v.itemcode === this.list[this.currency].itemcode)
					if (num >= 0) {
						this.itemList[num].fhsl = +this.fhsl
						this.itemList[num].price = +this.price
						this.itemList[num].account = +this.fhsl * +this.price
					} else {
						var item = {}
						...
						item.fhsl = this.list[this.currency].fhsl
						item.countt = this.list[this.currency].fhsl
						item.price = this.list[this.currency].price
						item.account = +this.list[this.currency].fhsl * +this.list[this.currency].price
