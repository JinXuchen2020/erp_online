	...
	<view class="content">
		...
		<u-popup v-model="searchShow" mode="center" width="666rpx" border-radius="14" :closeable="false">
			<view class="searchBox">
				<view class="searchTitle">输入发货信息</view>
				<view class="infoBox">
					...
					<u-button type="warning" class="searchBtn" :ripple="true" ripple-bg-color="#909399" :plain="true"
						size="medium" @click="selclientFun(khInfo)">选择客户</u-button>
				</view>
				<view class="dateBox">
					发货日期
				...
				<u-field v-model="khInfo.bz" :label="'发货备注'" :placeholder="'请输入发货备注'" clear-size="40"></u-field>
...
<script>
	...
	import delivergdsCard from '@/components/delivergds/delivergds.vue'

	import delivergdsproduct from '@/components/delivergds/delivergdsproduct.vue'
	...
	export default {
		...
		onLoad(options) {
			...
			if (that.cardIndex == -1) {
				...
				that.khInfo = uni.$delivergdsInfo;
			...
			if (uni.$delivergdsInfo) {
				that.djje = uni.$delivergdsInfo.account;
			}
			...
			uni.$on('deldelivergdsdetailById', that.deldelivergdsdetailById)
			...
			uni.$on('refreshDelivergdsFun', that.refreshDelivergdsFun)

			that.getDelivergdsdetailFun()
			that.getCurrencyFun();
			that.getSimpleFun();
			that.getTradeFun();
			that.getPostcodeFun();
		...
		onBackPress(e) {
			...
			uni.$off('refreshDelivergdsFun', that.refreshDelivergdsFun)
		...
		methods: {
			...
			// 通过下标更新list数据
			refreshDelivergdsFun: function(e) {
				that = this;
				that.khInfo = e.obj;
			...
			deldelivergdsdetailById: function(e) {
				var that = this;
				let hjje = 0;
			...
			// 获取发货详情记录
			getDelivergdsdetailFun: function() {
				uni.showLoading({
					title: '加载中...',
					mask: true
				})
