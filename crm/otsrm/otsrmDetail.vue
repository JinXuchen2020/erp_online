<template>
	<view class="content">
		...
		<z-paging ref="paging" :auto-clean-list-when-reload="false" :auto-scroll-to-top-when-reload="false"
			...
			<!--退货单基本信息-->
			<otsrmCard :item="khInfo" :isSelect="true" :djje="djje" :itemList="tabList[0].arr" :index="cardIndex"
				:pagetype="'退货详情'" :product="tabList[0].arr"></otsrmCard>
			<!--tab-->
			...
			<view class="contentView">
				...
				<view v-if="tabIndex == 0">
					<view v-for="(item, index) in tabList[0].arr" :key="index" @click="cardClick(item,index)">
						<otsrmproduct :item="item" :index="selectIndex" searchLabel1="下单数量" searchPh1="请输入下单数量"
							searchLabel2="产品售价" searchPh2="请输入产品售价"></otsrmproduct>
					</view>
				</view>
...
<script>
	...
	import otsrmCard from '@/components/otsrm/otsrm.vue'
	import otsrmproduct from '@/components/otsrm/otsrmproduct.vue'
	...
	export default {
		...
		onLoad(options) {
			...
			if (that.cardIndex == -1) {
				...
				that.khInfo = uni.$otsrmInfo;
			}

			if (uni.$otsrmInfo) {
				that.djje = uni.$otsrmInfo.account;
			}
			...
			uni.$on('delotsrmdetailById', that.delotsrmdetailById)
			...
			uni.$on('refreshOtsrmFun', that.refreshOtsrmFun)
			//if (that.isLoadSelectKhById) {
			//	that.selectKhByIdFun();
			//}
			that.getOtsrmdetailFun()
		},
		onBackPress(e) {
			uni.$off('delotsrmdetailById', that.delotsrmdetailById)
			...
			uni.$off('refreshOtsrmFun', that.refreshOtsrmFun)
