<template>
	<view>
		...
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectotsrmFun"
			refresher-enabled :refresher-threshold="200" :refresher-triggered="triggered" refresher-background="gray"
			@refresherrefresh="onRefresh" @refresherrestore="onRestore">
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index" >
					<otsrmCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'退货列表'"  @cxGetDataFun="cxGetDataFun"></otsrmCard>
				</view>
		...
		<addBtn url="./otsrmDetail?index=-1"></addBtn>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.selectOtsrmFun();
			uni.$on('addOtsrmFun', that.addOtsrmFun)
			uni.$on('deleteOtsrmFun', that.deleteOtsrmFun)
		...
		onBackPress() {
			uni.$off('addOtsrmFun', that.addOtsrmFun)
			uni.$off('deleteOtsrmFun', that.deleteOtsrmFun)
		...
		methods: {
			...
			deleteOtsrmFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addOtsrmFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addOtsrm);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addOtsrm)
				let arr = that.list;
				arr.unshift(uni.$addOtsrm);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
