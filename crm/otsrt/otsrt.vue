<template>
	<view>
		...
		<dataNull v-if="list.length == 0" src="/static/img/chahua/dataNullXz.png" title="暂无相关退货单"
			title1="请更换查询条件"></dataNull>
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectotsrtFun"
			refresher-enabled :refresher-threshold="200" :refresher-triggered="triggered" refresher-background="gray"
			@refresherrefresh="onRefresh" @refresherrestore="onRestore">
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index" >
					<otsrtCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'退货列表'"  @cxGetDataFun="cxGetDataFun"></otsrtCard>
				</view>
		...
		<addBtn url="./otsrtDetail?index=-1"></addBtn>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.selectOtsrtFun();
			uni.$on('addOtsrtFun', that.addOtsrtFun)
			uni.$on('deleteOtsrtFun', that.deleteOtsrtFun)
		...
		onBackPress() {
			uni.$off('addOtsrtFun', that.addOtsrtFun)
			uni.$off('deleteOtsrtFun', that.deleteOtsrtFun)
		...
		methods: {
			...
			deleteOtsrtFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addOtsrtFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addOtsrt);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addOtsrt)
				let arr = that.list;
				arr.unshift(uni.$addOtsrt);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
