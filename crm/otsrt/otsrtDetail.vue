<template>
	<view class="content">
		...
		<z-paging ref="paging" :auto-clean-list-when-reload="false" :auto-scroll-to-top-when-reload="false"
			...
			<!--退货单基本信息-->
			<otsrtCard :item="khInfo" :isSelect="true" :djje="djje" :itemList="tabList[0].arr" :index="cardIndex"
				:pagetype="'退货详情'" :product="tabList[0].arr"></otsrtCard>
			<!--tab-->
			...
			<view class="contentView">
				...
				<view v-if="tabIndex == 0">
					<view v-for="(item, index) in tabList[0].arr" :key="index" @click="cardClick(item,index)">
						<otsrtproduct :item="item" :index="selectIndex" searchLabel1="下单数量" searchPh1="请输入下单数量"
							searchLabel2="产品售价" searchPh2="请输入产品售价"></otsrtproduct>
					</view>
				</view>
...
<script>
	...
	import otsrtCard from '@/components/otsrt/otsrt.vue'
	import otsrtproduct from '@/components/otsrt/otsrtproduct.vue'
	...
	export default {
		...
		onLoad(options) {
			...
			if (that.cardIndex == -1) {
				...
				that.khInfo = uni.$otsrtInfo;
			}

			if (uni.$otsrtInfo) {
				that.djje = uni.$otsrtInfo.account;
			}
			...
			uni.$on('delotsrtdetailById', that.delotsrtdetailById)
			...
			uni.$on('refreshOtsrtFun', that.refreshOtsrtFun)
			//if (that.isLoadSelectKhById) {
			//	that.selectKhByIdFun();
			//}
			that.getOtsrtdetailFun()
		},
		onBackPress(e) {
			uni.$off('delotsrtdetailById', that.delotsrtdetailById)
			...
			uni.$off('refreshOtsrtFun', that.refreshOtsrtFun)
