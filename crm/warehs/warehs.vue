<template>
	<view>
		...
		<dataNull v-if="list.length == 0" src="/static/img/chahua/dataNullXz.png" title="暂无相关入库单"
			title1="请更换查询条件"></dataNull>
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectwarehsFun"
			refresher-enabled :refresher-threshold="200" :refresher-triggered="triggered" refresher-background="gray"
			@refresherrefresh="onRefresh" @refresherrestore="onRestore">
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index" >
					<warehsCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'入库列表'"  @cxGetDataFun="cxGetDataFun"></warehsCard>
				</view>
		...
		<addBtn url="./warehsDetail?index=-1"></addBtn>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.selectWarehsFun();
			uni.$on('addWarehsFun', that.addWarehsFun)
			uni.$on('deleteWarehsFun', that.deleteWarehsFun)
		...
		onBackPress() {
			uni.$off('addWarehsFun', that.addWarehsFun)
			uni.$off('deleteWarehsFun', that.deleteWarehsFun)
		...
		methods: {
			...
			deleteWarehsFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addWarehsFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addWarehs);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addWarehs)
				let arr = that.list;
				arr.unshift(uni.$addWarehs);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
