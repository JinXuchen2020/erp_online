<template>
	<view class="content">
		...
		<z-paging ref="paging" :auto-clean-list-when-reload="false" :auto-scroll-to-top-when-reload="false"
			...
			<!--入库单基本信息-->
			<warehsCard :item="khInfo" :isSelect="true" :djje="djje" :itemList="tabList[0].arr" :index="cardIndex"
				:pagetype="'入库详情'" :product="tabList[0].arr"></warehsCard>
			<!--tab-->
			...
			<view class="contentView">
				...
				<view v-if="tabIndex == 0">
					<view v-for="(item, index) in tabList[0].arr" :key="index" @click="cardClick(item,index)">
						<warehsproduct :item="item" :index="selectIndex" searchLabel1="下单数量" searchPh1="请输入下单数量"
							searchLabel2="产品售价" searchPh2="请输入产品售价"></warehsproduct>
					</view>
				</view>
...
<script>
	...
	import warehsCard from '@/components/warehs/warehs.vue'
	import warehsproduct from '@/components/warehs/warehsproduct.vue'
	...
	export default {
		...
		onLoad(options) {
			...
			if (that.cardIndex == -1) {
				...
				that.khInfo = uni.$warehsInfo;
			}

			if (uni.$warehsInfo) {
				that.djje = uni.$warehsInfo.account;
			}
			...
			uni.$on('delwarehsdetailById', that.delwarehsdetailById)
			...
			uni.$on('refreshWarehsFun', that.refreshWarehsFun)
			//if (that.isLoadSelectKhById) {
			//	that.selectKhByIdFun();
			//}
			that.getWarehsdetailFun()
		},
		onBackPress(e) {
			uni.$off('delwarehsdetailById', that.delwarehsdetailById)
			...
			uni.$off('refreshWarehsFun', that.refreshWarehsFun)
		...
		methods: {
			...
			delwarehsdetailById: function(e) {
				var that = this;
				let hjje = 0;
