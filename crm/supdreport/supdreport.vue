<template>
	<view>
		...
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectsupdreportFun"
			...
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index" @click="khCardClickFun(item)">
					<supdreportCard :item="item" :isSelect="isSelect" :index="index" @cxGetDataFun="cxGetDataFun"></supdreportCard>
				</view>
...
<script>
	...
	import supdreportCard from '@/components/card/supdreport.vue'
	export default {
		components: {
			dataNull,
			getMore,			
		...
		methods: {
			...
			khCardClickFun: function(item) {
				...
				if (that.pageType == 'lxr') {
					...
					uni.$khInfo = item;
					uni.$emit('supdreportBindFun', item)
					uni.navigateBack()
				}
