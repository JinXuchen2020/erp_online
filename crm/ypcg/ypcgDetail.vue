<template>
	<view class="content">
		...
		<z-paging ref="paging" :auto-clean-list-when-reload="false" :auto-scroll-to-top-when-reload="false"
			...
			<!--订单基本信息-->
			<ypcgCard :item="khInfo" :isSelect="true" :djje="djje" :itemList="tabList[0].arr" :index="cardIndex" :priceRight="priceRight"
				:pagetype="'订单详情'" :product="tabList[0].arr"></ypcgCard>
			<!--tab-->
			...
			<view class="contentView">
				...
				<view v-if="tabIndex == 0">
					<view v-for="(item, index) in tabList[0].arr" :key="index" @click="cardClick(item,index)">
						<ypcgproduct :item="item" :index="selectIndex" :priceRight="priceRight" searchLabel1="下单数量" searchPh1="请输入下单数量"
							searchLabel2="产品售价" searchPh2="请输入产品售价"></ypcgproduct>
					</view>
				</view>
...
<script>
	...
	import ypcgCard from '@/components/ypcg/ypcg.vue'
	import ypcgproduct from '@/components/ypcg/ypcgproduct.vue'
	...
	export default {
		...
		onLoad(options) {
			...
			console.log(uni.$ypcgInfo)

			if (uni.$ypcgInfo) {
				that.djje = uni.$ypcgInfo.account;
			}
			...
			uni.$on('delypcgdetailById', that.delypcgdetailById)
			...
			uni.$on('refreshYpcgFun', that.refreshYpcgFun)
			
			that.getYpcgdetailFun();
			that.getCurrencyFun();
			that.getSimpleFun();
			that.getCkmcFun();
			that.getPostcodeFun();
		...
		onBackPress(e) {
			uni.$off('delypcgdetailById', that.delypcgdetailById)
			...
			uni.$off('refreshYpcgFun', that.refreshYpcgFun)
