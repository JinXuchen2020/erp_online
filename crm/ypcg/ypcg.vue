<template>
	<view>
		...
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectYpcgFun"
			...
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="item.F_BillID" >
					<ypcgCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'订单列表'" :isCheck="item.sh" :priceRight="priceRight"  @cxGetDataFun="cxGetDataFun"></ypcgCard>
				</view>
		...
		<addBtn url="./ypcgDetail?index=-1"></addBtn>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.cxGetDataFun();
			that.getRightFun();
			uni.$on('addYpcgFun', that.addYpcgFun)
			uni.$on('deleteYpcgFun', that.deleteYpcgFun)
		...
		onBackPress() {
			uni.$off('addYpcgFun', that.addYpcgFun)
			uni.$off('deleteYpcgFun', that.deleteYpcgFun)
		...
		methods: {
			...
			deleteYpcgFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addYpcgFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addYpcg);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addYpcg)
				let arr = that.list;
				arr.unshift(uni.$addYpcg);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
