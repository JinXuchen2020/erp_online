	...
	<view>
		...
		<uni-popup ref="popucountt" mode="center" width="666rpx" border-radius="14" :closeable="false">
			<view class="searchBox">
				...
				<u-field v-model="cgsl" :label="'下单数量'" :placeholder="'请输入下单数量'" clear-size="40"></u-field>
	...
	export default {
		...
		methods: {
			...
			delcountt(index) {
				...
				var num = this.itemList.findIndex(v =>((v.itemcode === this.list[index].itemcode)&&(v.dyxs === this.list[index].F_BillID)) )
			...
			inputcountFun: function() {
				...
				var num = this.itemList.findIndex(v => ((v.itemcode === this.list[this.currency].itemcode)&&(v.dyxs === this.list[this.currency].F_BillID)))
				console.log("a", num);
				if (num >= 0) {
					...
					item.dyxs = this.list[this.currency].F_BillID
					...
					.then(res => {
						...
						for (var i = 0; i < data.length; i++) {
							for (var j = 0; j < itemList.length; j++) {
								if ((itemList[j].itemcode == data[i].itemcode)&&(itemList[j].dyxs == data[i].F_BillID)){
