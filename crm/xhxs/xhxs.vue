<template>
	<view>
		...
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectXhxsFun"
			...
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="item.F_BillID" >
					<xhxsCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'订单列表'" :isCheck="item.sh" :priceRight="priceRight"  @cxGetDataFun="cxGetDataFun"></xhxsCard>
				</view>
		...
		<addBtn url="./xhxsDetail?index=-1"></addBtn>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.cxGetDataFun();
			that.getRightFun();
			uni.$on('addXhxsFun', that.addXhxsFun)
			uni.$on('deleteXhxsFun', that.deleteXhxsFun)
		...
		onBackPress() {
			uni.$off('addXhxsFun', that.addXhxsFun)
			uni.$off('deleteXhxsFun', that.deleteXhxsFun)
		...
		methods: {
			...
			deleteXhxsFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addXhxsFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addXhxs);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addXhxs)
				let arr = that.list;
				arr.unshift(uni.$addXhxs);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
