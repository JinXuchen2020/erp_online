	...
	<view>
		...
		<scroll-view v-else scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="getChanPinFun"
			...
			<view v-for="(item, index) in list" :key="index" @click="cardClick(item)">
				<view class="myCard">
					<view class="cardTopName">产品编号：{{item.code}}</view>
					<view class="myCard3">
						...
						<view class="infoBox">
							...
							<view class="cardRow" v-if="item.ckmc">
								<view>所属仓库：</view>
								<view><text class="redColor">{{item.ckmc}}</text></view>
							</view>
							<view class="cardRow" v-if="item.kc">
								<view>库存数量：</view>
								<view><text class="redColor">{{item.kc}}</text>{{item.unit}}</view>
							</view>							
	...
	export default {
		...
		methods: {
			...
			delcountt(index) {
				...
				var num = this.itemList.findIndex(v => v.itemcode === this.list[index].code)
			...
			inputcountFun: function() {
				if (this.countt > this.list[this.currency].kc) {
					uni.showModal({
						title: '提示',
						content: '发货数量大于库存数量',
						showCancel: false
				...
				else
				{
					...
					var num = this.itemList.findIndex(v => ((v.itemcode === this.list[this.currency].code)&&(v.ckmc === this.list[this.currency].ckmc)))
					console.log("a", num);
					if (num >= 0) {
						...
						item.itemcode = this.list[this.currency].code
					...
					.then(res => {
						...
						for (var i = 0; i < data.length; i++) {
							for (var j = 0; j < itemList.length; j++) {
								if ((itemList[j].itemcode == data[i].code)&&(itemList[j].ckmc == data[i].ckmc)) {
