		...
		methods: {
			...
			saveorderFun: function(e) {
				...
				testrightApi(reqData)
					.then(res => {
						let showTitle = res.data.tag
						let newBill = res.data.newBill
						let editBill = res.data.editBill
						let delBill = res.data.delBill
						let printBill = res.data.printBill
						let checkBill = res.data.checkBill
						let saveright = false;
						...
						if (saveright == true) {
							...
							let reqObj = {
								...
								xsxd_content: this.tabList[0].arr,
								djlx: '正常订单',
								wxlist:this.wxlist,
								fjlist:this.fjlist,
								xjlist:this.xjlist,
								...
								gnlist:this.gnlist,
								dqlist:this.dqlist,
								actdate: new Date(this.khInfo.actdate).setHours(new Date(this.khInfo.actdate).getHours() + 8),
								enddate: new Date(this.khInfo.enddate).setHours(new Date(this.khInfo.enddate).getHours() + 8),
							}
							uni.showLoading({
								title: '正在保存单据',
								mask: true
			...
			previewPrintfile(e) {
				setTimeout(()=>{
					uni.navigateTo({
						url: './cgzjYhPreview?reportName=' + e.item.F_ReportName
					})
				}, 1000)				
			},
		}
