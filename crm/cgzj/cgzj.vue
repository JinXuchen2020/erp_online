<template>
	<view>
		...
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectCgzjFun"
...
<script>
	...
	export default {
		...
		onBackPress() {
			...
			uni.$off('deleteCgzjFun', that.deleteCgzjFun)
		...
		methods: {
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addCgzj)
				let arr = that.list;
				arr.unshift(uni.$addCgzj);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
