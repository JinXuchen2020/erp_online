<template>
	<view class="content">
		...
		<z-paging ref="paging" :auto-clean-list-when-reload="false" :auto-scroll-to-top-when-reload="false"
			...
			<!--订单基本信息-->
			<ypxsCard :item="khInfo" :isSelect="true" :djje="djje" :itemList="tabList[0].arr" :index="cardIndex" :priceRight="priceRight"
				:pagetype="'订单详情'" :product="tabList[0].arr"></ypxsCard>
			<!--tab-->
			...
			<view class="contentView">
				...
				<view v-if="tabIndex == 0">
					<view v-for="(item, index) in tabList[0].arr" :key="index" @click="cardClick(item,index)">
						<ypxsproduct :item="item" :index="selectIndex" :priceRight="priceRight" searchLabel1="下单数量" searchPh1="请输入下单数量"
							searchLabel2="产品售价" searchPh2="请输入产品售价"></ypxsproduct>
					</view>
				</view>
...
<script>
	...
	import ypxsCard from '@/components/ypxs/ypxs.vue'
	import ypxsproduct from '@/components/ypxs/ypxsproduct.vue'
	...
	export default {
		...
		onLoad(options) {
			...
			console.log(uni.$ypxsInfo)

			if (uni.$ypxsInfo) {
				that.djje = uni.$ypxsInfo.account;
			}
			...
			uni.$on('delypxsdetailById', that.delypxsdetailById)
			...
			uni.$on('refreshYpxsFun', that.refreshYpxsFun)
			
			that.getYpxsdetailFun();
			that.getCurrencyFun();
			that.getSimpleFun();
			that.getTradeFun();
			that.getPostcodeFun();
		...
		onBackPress(e) {
			uni.$off('delypxsdetailById', that.delypxsdetailById)
			...
			uni.$off('refreshYpxsun', that.refreshYpxsFun)
