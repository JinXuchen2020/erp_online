<template>
	<view>
		...
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectYpxsFun"
			...
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="item.F_BillID" >
					<ypxsCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'订单列表'" :isCheck="item.sh" :priceRight="priceRight"  @cxGetDataFun="cxGetDataFun"></ypxsCard>
				</view>
		...
		<addBtn url="./ypxsDetail?index=-1"></addBtn>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.cxGetDataFun();
			that.getRightFun();
			uni.$on('addYpxsFun', that.addYpxsFun)
			uni.$on('deleteYpxsFun', that.deleteYpxsFun)
		...
		onBackPress() {
			uni.$off('addYpxsFun', that.addYpxsFun)
			uni.$off('deleteYpxsFun', that.deleteYpxsFun)
		...
		methods: {
			...
			deleteYpxsFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addYpxsFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addYpxs);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addYpxs)
				let arr = that.list;
				arr.unshift(uni.$addYpxs);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
