<template>
	<view class="content">
		...
		<z-paging ref="paging" :auto-clean-list-when-reload="false" :auto-scroll-to-top-when-reload="false"
			...
			<!--客户基本信息-->
			<clientsreportCard :item="khInfo" :index="cardIndex" :isSelect="true"></clientsreportCard>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			uni.$on('refreshClientsreportFun', that.refreshClientsreportFun)
		},
		onBackPress(e) {
			...
			uni.$off('refreshClientsreportFun', that.refreshClientsreportFun)
		},
		onShow() {
		...
		methods: {
			// 通过下标更新list数据
			refreshClientsreportFun: function(e) {
				that = this;
				that.khInfo = e.obj;
