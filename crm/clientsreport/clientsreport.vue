<template>
	<view>
		...
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectclientsreportFun"
			...
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index" @click="khCardClickFun(item)">
					<clientsreportCard :item="item" :isSelect="isSelect" :index="index" @cxGetDataFun="cxGetDataFun"></clientsreportCard>
				</view>
...
<script>
	...
	import {
		getDayFun, getDayByNumFun,getMonthFun
	} from '@/utils/date.js'
	import dataNull from '@/components/dataNull/dataNull.vue'
	
	import getMore from '@/components/getMore/getMore.vue'
	
	import clientsreportCard from '@/components/card/clientsreport.vue'
	export default {
		components: {
			dataNull,
			getMore,			
		...
		methods: {
			...
			// 查询客户
			selectclientsreportFun: function() {
				if (!that.isMore) {
					return
				}
				uni.showLoading({
				...
				let reqObj = {
					...
					model:'frmClientReport1'
				}
				let reqData = {
					action: 'selectClientsreport',
			...
			khCardClickFun: function(item) {
				...
				if (that.pageType == 'lxr') {
					...
					uni.$khInfo = item;
					uni.$emit('clientsreportBindFun', item)
					uni.navigateBack()
				}
			...
			addItemInListFun: function() {
				...
				let arr = that.list;
				arr.unshift(uni.$addClientsreport);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
