<template>
	<view>
		...
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectPurorderFun"
			...
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="item.F_BillID" >
					<purorderCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'订单列表'" :isCheck="item.sh" :priceRight="priceRight"  @cxGetDataFun="cxGetDataFun"></purorderCard>
				</view>
		...
		<addBtn url="./purorderDetail?index=-1"></addBtn>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.cxGetDataFun();
			that.getRightFun();
			uni.$on('addPurorderFun', that.addPurorderFun)
			uni.$on('deletePurorderFun', that.deletePurorderFun)
		...
		onBackPress() {
			uni.$off('addPurorderFun', that.addPurorderFun)
			uni.$off('deletePurorderFun', that.deletePurorderFun)
		...
		methods: {
			...
			deletePurorderFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addPurorderFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addPurorder);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addPurorder)
				let arr = that.list;
				arr.unshift(uni.$addPurorder);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
