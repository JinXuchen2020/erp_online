<template>
	<view>
		...
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectYpzjFun"
			...
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="item.F_BillID" >
					<ypzjCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'订单列表'" :isCheck="item.sh" :priceRight="priceRight"  @cxGetDataFun="cxGetDataFun"></ypzjCard>
				</view>
		...
		<addBtn url="./ypzjDetail?index=-1"></addBtn>
...
<script>
	...
	import ypzjCard from '@/components/ypzj/ypzj.vue'
	export default {
		components: {
			dataNull,
			addBtn,
		...
		onLoad(e) {
			...
			that.cxGetDataFun();
			that.getRightFun();
			uni.$on('addYpzjFun', that.addYpzjFun)
			uni.$on('deleteYpzjFun', that.deleteYpzjFun)
		...
		onBackPress() {
			uni.$off('addYpzjFun', that.addYpzjFun)
			uni.$off('deleteYpzjFun', that.deleteYpzjFun)
		...
		methods: {
			...
			deleteYpzjFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addYpzjFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addYpzj);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addYpzj)
				let arr = that.list;
				arr.unshift(uni.$addYpzj);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
