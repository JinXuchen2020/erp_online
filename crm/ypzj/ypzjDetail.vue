...
<script>
	...
	import ypzjproduct from '@/components/ypzj/ypzjproduct.vue'
	...
	export default {
		...
		onLoad(options) {
			...
			console.log(uni.$ypzjInfo)

			if (uni.$ypzjInfo) {
				that.djje = uni.$ypzjInfo.account;
			}
			...
			uni.$on('delypzjdetailById', that.delypzjdetailById)
			...
			uni.$on('refreshYpzjFun', that.refreshYpzjFun)
			
			that.getYpzjdetailFun();
			that.getCurrencyFun();
			that.getSimpleFun();
			that.getCkmcFun();
			that.getPostcodeFun();
		...
		onBackPress(e) {
			...
			uni.$off('refreshYpzjFun', that.refreshYpzjFun)
