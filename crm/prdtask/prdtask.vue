<template>
	<view>
		...
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectprdtaskFun"
			refresher-enabled :refresher-threshold="200" :refresher-triggered="triggered" refresher-background="gray"
			@refresherrefresh="onRefresh" @refresherrestore="onRestore">
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index" >
					<prdtaskCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'计划列表'"  @cxGetDataFun="cxGetDataFun"></prdtaskCard>
				</view>
		...
		<addBtn url="./prdtaskDetail?index=-1"></addBtn>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.selectPrdtaskFun();
			uni.$on('addPrdtaskFun', that.addPrdtaskFun)
			uni.$on('deletePrdtaskFun', that.deletePrdtaskFun)
		...
		onBackPress() {
			uni.$off('addPrdtaskFun', that.addPrdtaskFun)
			uni.$off('deletePrdtaskFun', that.deletePrdtaskFun)
		...
		methods: {
			...
			deletePrdtaskFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addPrdtaskFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addPrdtask);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addPrdtask)
				let arr = that.list;
				arr.unshift(uni.$addPrdtask);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
