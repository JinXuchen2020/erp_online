<template>
	<view class="content">
		...
		<z-paging ref="paging" :auto-clean-list-when-reload="false" :auto-scroll-to-top-when-reload="false"
			...
			<!--计划单基本信息-->
			<prdtaskCard :item="khInfo" :isSelect="true" :djje="djje" :itemList="tabList[0].arr" :index="cardIndex"
				:pagetype="'计划详情'" :product="tabList[0].arr"></prdtaskCard>
			<!--tab-->
			...
			<view class="contentView">
				...
				<view v-if="tabIndex == 0">
					<view v-for="(item, index) in tabList[0].arr" :key="index" @click="cardClick(item,index)">
						<prdtaskproduct :item="item" :index="selectIndex" searchLabel1="下单数量" searchPh1="请输入下单数量"
							searchLabel2="产品售价" searchPh2="请输入产品售价"></prdtaskproduct>
					</view>
				</view>
...
<script>
	...
	import prdtaskCard from '@/components/prdtask/prdtask.vue'	
	import prdtaskproduct from '@/components/prdtask/prdtaskproduct.vue'
	...
	export default {
		...
		onLoad(options) {
			...
			if (that.cardIndex == -1) {
				...
				that.khInfo = uni.$prdtaskInfo;
			}

			if (uni.$prdtaskInfo) {
				that.djje = uni.$prdtaskInfo.account;
			}
			...
			uni.$on('delprdtaskdetailById', that.delprdtaskdetailById)
			...
			uni.$on('refreshPrdtaskFun', that.refreshPrdtaskFun)
			//if (that.isLoadSelectKhById) {
			//	that.selectKhByIdFun();
			//}
			that.getPrdtaskdetailFun()
		},
		onBackPress(e) {
			uni.$off('delprdtaskdetailById', that.delprdtaskdetailById)
			...
			uni.$off('refreshPrdtaskFun', that.refreshPrdtaskFun)
		...
		methods: {
			...
			delprdtaskdetailById: function(e) {
				var that = this;
				let hjje = 0;
