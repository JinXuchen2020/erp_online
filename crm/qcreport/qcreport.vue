<template>
	<view>
		...
		<dataNull v-if="list.length == 0" src="/static/img/chahua/dataNullXz.png" title="暂无相关供方"
			title1="请更换查询条件"></dataNull>
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectqcreportFun"
			refresher-enabled :refresher-threshold="200" :refresher-triggered="triggered" refresher-background="gray"
			@refresherrefresh="onRefresh" @refresherrestore="onRestore">
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index" >
					<qcorderCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'订单列表'"  @cxGetDataFun="cxGetDataFun"></qcorderCard>
				</view>
		...
		<addBtn url="./qcorderDetail?index=-1"></addBtn>
...
<script>
	...
	import qcorderCard from '@/components/qcorder/qcorder.vue'
	export default {
		components: {
			dataNull,
			addBtn,
		...
		onLoad(e) {
			...
			that.selectQcorderFun();
			uni.$on('addQcorderFun', that.addQcorderFun)
			uni.$on('deleteQcorderFun', that.deleteQcorderFun)
		...
		onBackPress() {
			uni.$off('addQcorderFun', that.addQcorderFun)
			uni.$off('deleteQcorderFun', that.deleteQcorderFun)
		...
		methods: {
			...
			deleteQcorderFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addQcorderFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addQcorder);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addQcorder)
				let arr = that.list;
				arr.unshift(uni.$addQcorder);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
