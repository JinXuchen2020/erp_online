	...
	<view class="content">
		...
		<u-popup v-model="searchShow" mode="center" width="666rpx" border-radius="14" :closeable="false">
			<view class="searchBox">
				...
				<!-- <view class="infoBox">
					<u-field v-model="khInfo.fkfs" :label="'付款方式'" :placeholder="'请选择付款方式'" clear-size="40"
						class="clientBox"></u-field>
					<u-button type="warning" class="searchBtn" :ripple="true" ripple-bg-color="#909399"
						:plain="true" size="medium" @click="selfkfsFun(khInfo.clientcode)">选择</u-button>
				</view> -->
...
<script>
	...
	import PIproduct from '@/components/PI/PIproduct.vue'
	...
	export default {
		...
		onLoad(options) {
			...
			console.log(uni.$PIInfo)

			if (uni.$PIInfo) {
				that.djje = uni.$PIInfo.account;
			}
			...
			uni.$on('delPIdetailById', that.delPIdetailById)
			...
			uni.$on('refreshPIFun', that.refreshPIFun)
		...
		onBackPress(e) {
			...
			uni.$off('refreshPIFun', that.refreshPIFun)
