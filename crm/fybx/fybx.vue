<template>
	<view>
		...
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectFybxFun"
			refresher-enabled :refresher-threshold="200" :refresher-triggered="triggered" refresher-background="gray"
			@refresherrefresh="onRefresh" @refresherrestore="onRestore">
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index" >
					<fybxCard :item="item" :index="index" :isSelect="isSelect"  :pagetype="'付款列表'"  @cxGetDataFun="cxGetDataFun"></fybxCard>
				</view>
		...
		<addBtn url="./fybxDetail?index=-1"></addBtn>
		
		<!--底部合计-->
		<view class="submitView">
			<view class="cardTopName disFlexJ">
				<text>合计：{{gs}}个单 付款￥: {{djje}}元 </text>
			</view>
		</view>
	</view>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.selectFybxFun();
			uni.$on('addFybxFun', that.addFybxFun)
			uni.$on('deleteFybxFun', that.deleteFybxFun)
		...
		onBackPress() {
			uni.$off('addFybxFun', that.addFybxFun)
			uni.$off('deleteFybxFun', that.deleteFybxFun)
		...
		methods: {
			...
			deleteFybxFun: function(e) {
				
				let arr = that.list;
				arr.splice(e.index, 1);
				that.list = arr;
			},
			addFybxFun: function(e) {			
				
				let arr = that.list;
				arr.concat(uni.$addFybx);
				that.list = arr;
			...
			addItemInListFun: function(e) {
				console.log('新增传递过来的数组：',uni.$addFybx)
				let arr = that.list;
				arr.unshift(uni.$addFybx);
				that.list = arr;
				uni.pageScrollTo({
					scrollTop: 0
				})
