<template>
	<view>
		...
		<dataNull v-if="list.length == 0" src="/static/img/chahua/dataNullXz.png" title="暂无相关云打印机"
			title1="请更换查询条件"></dataNull>
		<scroll-view scroll-y="true" :style="{height: scrollHeight}" @scrolltolower="selectprinterFun"
			refresher-enabled :refresher-threshold="200" :refresher-triggered="triggered" refresher-background="gray"
			@refresherrefresh="onRefresh" @refresherrestore="onRestore">
			<view v-if="list.length > 0">
				<view v-for="(item, index) in list" :key="index" >
					<selPrinter :item="item" :isSelect="isSelect" :index="index" @cxGetDataFun="cxGetDataFun"></selPrinter>
				</view>
...
<script>
	...
	import selPrinter from '../../components/salesorder/selPrinter.vue'
	export default {
		components: {
			dataNull,
			addBtn,
