		...
		<view class="card">
			<view class="topRow1" @click="gotoDetailFun">
				...
				<view v-if="item.actdate">
					<text class="colorGray">退货日期：</text>
			...
			<view class="rowBtn">
				<u-button v-if='(pagetype === "退货详情")&&(checkbill !== true)' type="primary" :plain="true" class="cpBtn"
				...
				<u-button v-if='(pagetype === "退货详情")&&(item.F_BillID)' type="warning" :plain="true" class="cpBtn"
					size="mini" @click="selprintfileFun">
					打印
				</u-button>
				<u-button type="error" :plain="true" class="cpBtn" size="mini"
					@click="deleteSalesreturnFun(item, index)">
					删除</u-button>
			</view>
			<!--搜索弹窗-->
			<u-popup v-model="searchShow" mode="center" width="666rpx" border-radius="14" :closeable="false">
				<view class="searchBox">
					<view class="searchTitle">输入退货信息</view>
					<view class="infoBox">
						<u-field v-model="item.clientname" :label="'客户名称'" :placeholder="'请输入下单客户'" clear-size="40"
					...
					<view class="infoBox">
						<u-field v-model="item.fkfs" :label="'付款方式'" :placeholder="'请选择付款方式'" clear-size="40"
							class="clientBox"></u-field>
						<u-button type="warning" class="searchBtn" :ripple="true" ripple-bg-color="#909399"
							:plain="true" size="medium" @click="selfkfsFun(item.clientcode)">选择</u-button>


					</view>
					<u-field v-model="item.postcode" :label="'送货地址'" :placeholder="'请选择送货地址'" clear-size="40"></u-field>
					<u-field v-model="item.bz" :label="'退货备注'" :placeholder="'请输入退货备注'" clear-size="40"></u-field>
	...
	export default {
		...
		data() {
			return {
				...
				enddate: this.$u.timeFormat(this.item.enddate, 'yyyy-mm-dd'),
				fkfs: this.item.fkfs,
		...
		methods: {
			...
			selclientFun: function(e) {
				...
				uni.navigateTo({
					url: './salesreturnselClient'
				})
				
			},
			// 选择付款方式函数
			selfkfsFun: function(e) {
				uni.navigateTo({
					url: './salesreturnselPayment'
				})
			},
			// 选择打印文件
							...
							salesreturnApi(reqData)
								.then(res => {
									...
									if (this.item.F_BillID == '') {
										...
										uni.$addSalesreturn = res.data.rows[0];
										...
										uni.$emit('refreshSalesreturnFun', {
											index: this.cardIndex,
											obj: res.data.rows[0]
										})
									}
			...
			gotoDetailFun: function() {
				...
				uni.$salesreturnInfo = this.item;
				uni.navigateTo({
					url: './salesreturnDetail?index=' + this.index
				})
			},
			...
			// 删除退货
			deleteSalesreturnFun: function(item, index) {
				var that = this;
				let delright = false;
				//确认权限
				let reqObjright = {
				...
				testrightApi(reqDataright)
					.then(res => {
						...
						if (delright) {
							uni.showModal({
								title: '提示',
								content: '是否确认删除退货：' + item.F_BillID,
								success(rrr) {
									if (rrr.confirm) {
										uni.showLoading({
											title: '删除中...'
										...
										salesreturnApi(reqData)
											.then(res => {
												...
												uni.$emit('deleteSalesreturnFun', {
													index,
												})

												if (that.pagetype == "退货详情") {
													setTimeout(() => {
														uni.navigateBack()
													}, 666)
												}
