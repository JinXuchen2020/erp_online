		...
		<view class="card">
			<view class="topRow1" @click="gotoDetailFun">
				...
				<view v-if="item.actdate">
					<text class="colorGray">下单日期：</text>
			...
			<view class="rowBtn">
				<u-button v-if='(pagetype === "订单详情")&&(checkbill !== true)' type="primary" :plain="true" class="cpBtn"
				...
				<u-button type="error" :plain="true" class="cpBtn" size="mini"
					@click="deletePurorderFun(item, index)">
					删除</u-button>
			</view>
			<!--搜索弹窗-->
			<u-popup v-model="searchShow" mode="center" width="666rpx" border-radius="14" :closeable="false">
				<view class="searchBox">
					...
					<view class="infoBox">
						<u-field v-model="item.clientname" :label="'供方名称'" :placeholder="'请输入下单供方'" clear-size="40"
							class="clientBox"></u-field>
						<u-button type="warning" class="searchBtn" :ripple="true" ripple-bg-color="#909399"
							:plain="true" size="medium" @click="selclientFun(item)">选择供方</u-button>
					...
					<u-field v-model="item.bz" :label="'订单备注'" :placeholder="'请输入订单备注'" clear-size="40"></u-field>
	...
	export default {
		...
		methods: {
							...
							purorderApi(reqData)
								.then(res => {
									...
									if (this.item.F_BillID == '') {
										...
										uni.$addPurorder = res.data.rows[0];
			...
			gotoDetailFun: function() {
				...
				uni.$purorderInfo = this.item;
				uni.navigateTo({
					url: './purorderDetail?index=' + this.index
				})
			},
			...
			// 删除订单
			deletePurorderFun: function(item, index) {
				var that = this;
				let delright = false;
				//确认权限
				let reqObjright = {
