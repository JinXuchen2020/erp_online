	...
	<view>
		<view class="card">
			<view class="topRow1" >
                ...
                <view v-if="item.cpbh" >
					<text class="colorGray">产品编号：</text>
					<text>{{item.cpbh}}</text>
				</view>
				<view v-if="item.cpmc">
					<text class="colorGray">产品名称：</text>
					<text>{{item.cpmc}}</text>
				</view>
				<view v-if="item.sl">
					<text class="colorGray">生产数量：</text>
					<text>{{item.sl}}</text>
				</view>
				<view v-if="item.itemname">
					<text class="colorGray">工序名称：</text>
					<text>{{item.itemname}}</text>
				</view>
				<view v-if="item.countt">
					<text class="colorGray">工序数量：</text>
					<text>{{item.countt}}</text>
				</view>
				<view v-if="item.count1">
					<text class="colorGray">完工数量：</text>
					<text>{{item.count1}}</text>
				</view>
				<view v-if="item.count2">
					<text class="colorGray">差额数量：</text>
					<text>{{item.count2}}</text>
				</view>
				<view v-if="item.pre_sl_count2">
					<text class="colorGray">可登数量：</text>
					<text>{{item.pre_sl_count2}}</text>
				</view>
				<view v-if="item.actdate">
					<text class="colorGray">生产日期：</text>
					<text>{{item.actdate}}</text>
				</view>				
				<view v-if="item.rwdh">
					<text class="colorGray">生产单号：</text>
					<text>{{item.rwdh}}</text>
				</view>				
				<view v-if="item.bz">
					<text class="colorGray">生产备注：</text>
					<text>{{item.bz}}</text>
				</view>
				<u-field v-model="item.wgsl" :label="'完工数量'" :placeholder="'请输入完工数量'" clear-size="40"></u-field>
                    <view class="infoBox">
			...
			<view class="rowBtn">
				...
				<u-button v-if='item.pre_sl_count2 >0' type="primary" :plain="true" class="cpBtn"
					size="mini" @click="saveorderFun">保存
				</u-button>			
				
				<u-button type="error" :plain="true" class="cpBtn" size="mini" @click="deletePrdfinFun(item, index)">
					取消</u-button>
			</view>
			<!--搜索弹窗-->
			<u-popup v-model="searchShow" mode="center" width="666rpx" border-radius="14" :closeable="false">
				<view class="searchBox">
					<view class="searchTitle">输入完工信息</view>
					<view class="dateBox">
						完工日期
					...
					<u-field v-model="item.bz" :label="'计划备注'" :placeholder="'请输入计划备注'" clear-size="40"></u-field>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 删除计划
			deletePrdfinFun: function(item, index) {
				var that = this;
				let delright = false;
				//确认权限
				let reqObjright = {
				...
				testrightApi(reqDataright)
					.then(res => {
						...
						if (delright) {
							uni.showModal({
								title: '提示',
								content: '是否确认删除计划：' + item.F_BillID,
								success(rrr) {
									if (rrr.confirm) {
										uni.showLoading({
											title: '删除中...'
										...
										prdfinApi(reqData)
											.then(res => {
												...
												uni.$emit('deletePrdfinFun', {
													index,
												})

												if (that.pagetype == "计划详情") {
													setTimeout(() => {
														uni.navigateBack()
													}, 666)
												}
