	...
	<view>
		<view class="card">
			<view class="topRow1" @click="gotoDetailFun">
				<view class="name">
					...
					<view class="rowBtn">
						<u-button type="primary" :plain="true" class="cpBtn" size="mini"
							@click="selClientFun(item, index)">选择单据
						</u-button>
				...
				<view v-if="item.bz">
					<text class="colorGray">发货备注：</text>
					<text>{{item.bz}}</text>
				</view>
				<view v-if="item.actdate">
					<text class="colorGray">发货日期：</text>
	...
	export default {
		...
		methods: {
			...
			selClientFun: function(item, index) {
				...
				uni.showModal({
					title: '提示',
					content: '是否确认选择单据：' + that.item.F_BillID,
					success(rrr) {
						if (rrr.confirm) {
