		...
		<view class="card">
			...
			<view class="rowBtn">
				...
				<u-button type="error" :plain="true" class="cpBtn" size="mini"
					@click="deleteYpxsFun(item, index)">
					删除</u-button>
			</view>	
		</view>
	</view>
	...
	export default {
		...
		methods: {
			...
			gotoDetailFun: function() {
				...
				uni.$ypxsInfo = this.item;
				uni.navigateTo({
					url: './ypxsDetail?index=' + this.index
				})
			},
			...
			// 删除订单
			deleteYpxsFun: function(item, index) {
				var that = this;
				let delright = false;
				//确认权限
				let reqObjright = {
