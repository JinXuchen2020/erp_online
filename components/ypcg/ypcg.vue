		...
		<view class="card">
			...
			<view class="rowBtn">
				...
				<u-button type="error" :plain="true" class="cpBtn" size="mini"
					@click="deleteYpcgFun(item, index)">
					删除</u-button>
			</view>	
		</view>
	</view>
	...
	export default {
		...
		methods: {
								...
								.then(res => {
									...
									if (this.item.F_BillID == '') {
										...
										if (this.editBill == true) {
											...
											uni.setStorageSync("itemList", this.itemList)
											uni.navigateTo({
												url: './ypcgselProduct?dykh=' + this.item.dykh,
			...
			gotoDetailFun: function() {
				...
				uni.$ypcgInfo = this.item;
				uni.navigateTo({
					url: './ypcgDetail?index=' + this.index
				})
			},
			...
			// 删除订单
			deleteYpcgFun: function(item, index) {
				var that = this;
				let delright = false;
				//确认权限
				let reqObjright = {
