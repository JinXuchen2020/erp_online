		...
		<view class="card">
			<view class="topRow1" @click="gotoDetailFun">
				...
				<view v-if="item.actdate">
					<text class="colorGray">进仓日期：</text>
				...
				<view v-if="item.ckmc">
					<text class="colorGray">仓库名称：</text>
					<text>{{item.ckmc}}</text>
				</view>
				<view v-if="item.account">
					<text class="colorGray">订单金额：</text>
			...
			<view class="rowBtn">
				<u-button v-if='(pagetype === "进仓详情")&&(checkbill !== true)' type="primary" :plain="true" class="cpBtn"
				...
				<u-button v-if='(pagetype === "进仓详情")&&(item.F_BillID)' type="warning" :plain="true" class="cpBtn"
					size="mini" @click="selprintfileFun">
					打印
				</u-button>
				<u-button type="error" :plain="true" class="cpBtn" size="mini" @click="deleteOtswhFun(item, index)">
					删除</u-button>
			</view>
			<!--搜索弹窗-->
			<u-popup v-model="searchShow" mode="center" width="666rpx" border-radius="14" :closeable="false">
				<view class="searchBox">
					<view class="searchTitle">输入进仓信息</view>
					<view class="dateBox">
						进仓日期
					...
					<view class="infoBox">
						<u-field v-model="item.clientname" :label="'委外厂家'" :placeholder="'请选择生产委外厂家'" clear-size="40"
					...
					<view class="infoBox">
						<u-field v-model="item.ckmc" :label="'仓库名称'" :placeholder="'请选择生产仓库名称'" clear-size="40"
							class="clientBox"></u-field>
						<u-button type="warning" class="searchBtn" :ripple="true" ripple-bg-color="#909399"
							:plain="true" size="medium" @click="selckmcFun(item.ckmc)">选择</u-button>
					</view>
					<u-field v-model="item.bz" :label="'进仓备注'" :placeholder="'请输入进仓备注'" clear-size="40"></u-field>
	...
	export default {
		...
		methods: {
						...
						.then(res => {
							...
							if (this.item.F_BillID == '') {
								if (this.newBill == true) {
									...
									uni.navigateTo({
										url: './otswhselProduct?ckmc=' + this.item.clientcode,
								...
								if (this.editBill == true) {
									...
									uni.setStorageSync("itemList", this.itemList)
									uni.navigateTo({
										url: './otswhselProduct?ckmc=' + this.item.ckmc,
			...
			gotoDetailFun: function() {
				...
				uni.$otswhInfo = this.item;
				uni.navigateTo({
					url: './otswhDetail?index=' + this.index
				})
			},
			...
			// 删除进仓
			deleteOtswhFun: function(item, index) {
				var that = this;
				let delright = false;
				//确认权限
				let reqObjright = {
				...
				testrightApi(reqDataright)
					.then(res => {
						...
						if (delright) {
							uni.showModal({
								title: '提示',
								content: '是否确认删除进仓：' + item.F_BillID,
								success(rrr) {
									if (rrr.confirm) {
										uni.showLoading({
											title: '删除中...'
										...
										otswhApi(reqData)
											.then(res => {
												...
												uni.$emit('deleteOtswhFun', {
													index,
												})

												if (that.pagetype == "进仓详情") {
													setTimeout(() => {
														uni.navigateBack()
													}, 666)
												}
