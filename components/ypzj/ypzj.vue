		...
		<view class="card">
			...
			<view class="rowBtn">
				...
				<u-button type="error" :plain="true" class="cpBtn" size="mini"
					@click="deleteYpzjFun(item, index)">
					删除</u-button>
			</view>	
		</view>
	</view>
	...
	export default {
		...
		methods: {
								...
								.then(res => {
									...
									if (this.item.F_BillID == '') {
										if (this.newBill == true) {
											...
											uni.navigateTo({
												url: './ypzjselProduct?dykh=' + this.item.dykh+'&&clientcode='+this.item.clientcode,
			...
			gotoDetailFun: function() {
				...
				uni.$ypzjInfo = this.item;
				uni.navigateTo({
					url: './ypzjDetail?index=' + this.index
				})
			},
			...
			// 删除订单
			deleteYpzjFun: function(item, index) {
				var that = this;
				let delright = false;
				//确认权限
				let reqObjright = {
