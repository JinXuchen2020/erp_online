		...
		<view class="card">
			<view class="topRow1" @click="gotoDetailFun">
				...
				<view v-if="item.actdate">
					<text class="colorGray">入库日期：</text>
					<text>{{item.actdate}}</text>
				</view>			
				
				<view v-if="item.zd">
					<text class="colorGray">入库人员：</text>
					<text>{{item.zd}}</text>
				</view>
				<view v-if="item.account">
					<text class="colorGray">入库金额：</text>
			...
			<view class="rowBtn">	
				<u-button v-if='(pagetype === "入库详情")&&(item.sh !== true)' type="primary" :plain="true" class="cpBtn"
			...
			<u-popup v-model="searchShow" mode="center" width="666rpx" border-radius="14" :closeable="false">
				<view class="searchBox">
					...
					<view class="infoBox">
						<u-field v-model="item.ckmc" :label="'仓库名称'" :placeholder="'请选择入库仓库'" clear-size="40"
					...
					<u-field v-model="item.bz" :label="'入库备注'" :placeholder="'请输入入库备注'" clear-size="40"></u-field>
	...
	export default {
		...
		methods: {
					...
					.then(res => {
						...
						if (this.item.F_BillID == '') {
							if (this.newBill == true) {
								...
								uni.navigateTo({
									url: './warehsselProduct?ckmc='+this.item.ckmc,
			...
			gotoDetailFun: function() {
				...
				uni.$warehsInfo = this.item;
				uni.navigateTo({
					url: './warehsDetail?index=' + this.index
				})
			},
			...
			// 删除入库
			deleteWarehsFun: function(item, index) {
				var that = this;
				let delright = false;
				//确认权限
				let reqObjright = {
				...
				testrightApi(reqDataright)
					.then(res => {
						...
						if (delright) {
							uni.showModal({
								title: '提示',
								content: '是否确认删除入库单：' + item.F_BillID,
								success(rrr) {
									if (rrr.confirm) {
										uni.showLoading({
											title: '删除中...'
										...
										warehsApi(reqData)
											.then(res => {
												...
												uni.$emit('deleteWarehsFun', {
													index,
												})

												if (that.pagetype == "入库详情") {
													setTimeout(() => {
														uni.navigateBack()
													}, 666)
												}
