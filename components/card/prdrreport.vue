	...
	<view>
		...
		<view class="card">
			<view class="topRow1" @click="gotoDetailFun">
				...
				<view >
					<text class="colorGray">收货数量：</text>
					<text>{{item.fhsl}} {{item.unit}}</text>
				</view>
				<view >
					<text class="colorGray">退货数量：</text>
					<text>{{item.thsl}} {{item.unit}}</text>
				</view>
				<view >
					<text class="colorGray">收货金额：</text>
	...
	export default {
		...
		methods: {
			...
			gotoDetailFun: function() {
				...
				uni.navigateTo({
					url: './prdrreportdetail?index=' + this.index
				})
			},		
		}
	}
