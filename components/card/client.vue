<template>
	<view>
		...
		<view class="card">
			...
			<view class="rowBtn">				
				<u-button type="warning" :plain="true" class="cpBtn" size="mini" @click="editClientFun(item, index)">
					编辑
				</u-button>
				<u-button type="error" :plain="true" class="cpBtn" size="mini"
					@click="deleteClientFun(item, index)">
					删除</u-button>
			</view>
		</view>
	</view>
	...
	export default {
		...
		methods: {
			...
			gotoDetailFun: function() {
				...
				uni.navigateTo({
					url: './clientdetail?index=' + this.index
				})
			},
			
			// 删除客户
			deleteClientFun: function(item, index) {
				var that = this;
				uni.showModal({
					title: '提示',
			...
			// 编辑客户
			editClientFun: function(item, index) {
				var that = this;
				uni.$addClient = this.item;
				console.log('addclient:'+uni.$addClient);
				console.log('addclient:'+this.item.name)
				uni.navigateTo({					
					url: './addclient?type=update&&isAdd=0&&index='+this.index
				})
			},
