...
<script>
	import { baobiaoApi } from '@/utils/api.js'
	...
	export default {
		...
		methods: {
			getServerData() {
				...
				that.isLoading = true;
				console.log('商机漏斗指令：'+reqData.action+'传递参数：'+reqData.params)
				baobiaoApi(reqData)
				.then(res => {
					that.isLoading = false;
					console.log('商机漏斗返回结果');					
					let arr = res.data.rows;
					
					console.log('判断length');
					if(arr.length == 0) {
						console.log('length为0')
					...
					for(var i = 0; i < arr.length; i++) {
						for(var j = 0; j < series.length; j++) {
							if(arr[i].sjJieduan == series[j].name) {
