...
<script>
	...
	export default {
		...
		methods: {
					...
					testrightApi(reqData)
						.then(res => {
							...
							if (this.printBill == true) {
								uni.navigateTo({
									url: './cgzjselPrintfile?type=print'
								})
							} else {
								uni.showModal({
									title: '提示',
								...
								.then(res => {
									...
									if (this.item.F_BillID == '') {
										if (this.newBill == true) {
											...
											uni.navigateTo({
												url: './cgzjselProduct?dykh=' + this.item.dykh+'&&clientcode='+this.item.clientcode,
			...
			// 删除订单
			deleteCgzjFun: function(item, index) {
				var that = this;
				let delright = false;
				//确认权限
				let reqObjright = {
