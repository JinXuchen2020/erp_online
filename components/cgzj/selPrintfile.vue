	...
	<view>
		<view class="card">
			<view class="topRow1" @click="gotoDetailFun">
				<view class="name">
					<text>{{item.F_ReportName || '暂无'}}</text>
					<view class="rowBtn">
						<u-button  type="primary" :plain="true" class="cpBtn" size="mini"
							@click="selPrintfileFun(item, index)">选择
						</u-button>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 选择打印样式
			selPrintfileFun: function(item, index) {
				var that = this;
				uni.showModal({
					title: '提示',
					content: '是否确认选择打印样式：' + that.item.F_ReportName,
					success(rrr) {
						if (rrr.confirm) {
							...
							if (that.pageType == 'print') {
								uni.$emit("selectPrintfile1",{"item":item})
							}
							else {								
								uni.$emit("previewPrintfile",{"item":item})
							}
							uni.navigateBack()
						}
					}
