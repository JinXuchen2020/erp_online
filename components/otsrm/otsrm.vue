	...
	<view>
		<view class="card">
			...
			<view class="rowBtn">
				...
				<u-button type="error" :plain="true" class="cpBtn" size="mini" @click="deleteOtsrmFun(item, index)">
					删除</u-button>
			</view>
			<!--搜索弹窗-->
	...
	export default {
		...
		methods: {
						...
						.then(res => {
							...
							if (this.item.F_BillID == '') {
								if (this.newBill == true) {
									...
									uni.navigateTo({
										url: './otsrmselProduct?clientcode=' + this.item.clientcode+'&&ckmc='+ this.item.ckmc,
			...
			gotoDetailFun: function() {
				...
				uni.$otsrmInfo = this.item;
				uni.navigateTo({
					url: './otsrmDetail?index=' + this.index
				})
			},
			...
			// 删除退货
			deleteOtsrmFun: function(item, index) {
				var that = this;
				let delright = false;
				//确认权限
				let reqObjright = {
