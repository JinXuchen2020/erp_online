	...
	<view>
		<view class="card">
			...
			<view class="rowBtn">
				...
				<u-button type="error" :plain="true" class="cpBtn" size="mini" @click="deletePrdtaskFun(item, index)">
					删除</u-button>
			</view>
			<!--搜索弹窗-->
	...
	export default {
		...
		methods: {
					...
					.then(res => {
						...
						if (this.item.F_BillID == '') {
							if (this.newBill == true) {
								...
								uni.navigateTo({
									url: './prdtaskselProduct?ckmc=' + this.item.ckmc,
							...
							prdtaskApi(reqData)
								.then(res => {
									...
									if (this.item.F_BillID == '') {
										...
										uni.$addPrdtask = res.data.rows[0];
			...
			gotoDetailFun: function() {
				...
				uni.$prdtaskInfo = this.item;
				uni.navigateTo({
					url: './prdtaskDetail?index=' + this.index
				})
			},
			...
			// 删除计划
			deletePrdtaskFun: function(item, index) {
				var that = this;
				let delright = false;
				//确认权限
				let reqObjright = {
