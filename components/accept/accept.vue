	...
	<view>
		<view class="card">
			<view class="topRow1" @click="gotoDetailFun">
				...
				<view class="infoBox">
					<view class="name">
						<text>{{item.F_ClientName || '暂无'}}</text>
					</view>
				...
				<view v-if="item.F_ClientID">
					<text class="colorGray">客户编号：</text>
					<text>{{item.F_ClientID}}</text>
				</view>
				<view v-if="item.F_Date">
					<text class="colorGray">收款日期：</text>
					<text>{{item.F_Date}}</text>
				</view>

				<view v-if="item.f_bank">
					<text class="colorGray">收款账户：</text>
					<text>{{item.f_bank}}</text>
				</view>				
				<view v-if="item.F_Money">
					<text class="colorGray">收款金额：</text>
				...
				<view v-if="item.F_dzje">
					<text class="colorGray">到账金额：</text>
				...
				<view v-if="item.F_Kind">
					<text class="colorGray">收款类型：</text>
					<text>{{item.F_Kind}}</text>
				</view>
				<view v-if="item.F_BillMan">
					<text class="colorGray">业务经手人：</text>
					<text>{{item.F_BillMan}}</text>
				</view>
			...
			<view class="rowBtn">
				<u-button v-if='(pagetype === "收款详情")&&(checkbill !== true)' type="primary" :plain="true" class="cpBtn"
				...
				<u-button type="warning" :plain="true" class="cpBtn" size="mini" @click="cpsxjFun(item, index)">
					{{checkbill == true ? '反审' : '审核'}}
					
				</u-button>
				<u-button v-if='(pagetype === "收款详情")&&(item.F_BillID)' type="warning" :plain="true" class="cpBtn"
					size="mini" @click="selprintfileFun">
					打印
				</u-button>
				<u-button type="error" :plain="true" class="cpBtn" size="mini" @click="deleteAcceptFun(item, index)">
					删除</u-button>
			</view>
			<!--搜索弹窗-->
			<!-- <u-popup v-model="searchShow" mode="center" width="666rpx" border-radius="14" :closeable="false">
				<view class="searchBox">
					...
					<view class="infoBox">
						<u-field v-model="item.f_bank" :label="'收款账户'" :placeholder="'请选择收款账户'" clear-size="40"
							class="clientBox"></u-field>
						<u-button type="warning" class="searchBtn" :ripple="true" ripple-bg-color="#909399"
							:plain="true" size="medium" @click="selbankFun(item.F_ClientID)">选择</u-button>


					</view>
					<u-field v-model="item.F_AcceptType" :label="'收款方式'" :placeholder="'请选择收款方式'"
						clear-size="40"></u-field>
					<u-field v-model="item.bz" :label="'收款备注'" :placeholder="'请输入收款备注'" clear-size="40"></u-field>
	...
	export default {
		...
		methods: {
						...
						if (this.item.F_BillID == '') {
							if (this.newBill == true) {
								...
								if (this.item.F_ClientID == '') {
									...
									uni.setStorageSync("acceptdetail", this.itemList)
									console.log('新增单据传递过去的itemlist', this.itemList)
									uni.navigateTo({
										url: './acceptselProduct?F_ClientID=' + this.item.F_ClientID,
			...
			gotoDetailFun: function() {
				...
				uni.$acceptInfo = this.item;
				uni.navigateTo({
					url: './acceptDetail?index=' + this.index
				})
			},
				...
				testrightApi(reqDataright)
					.then(res => {
						...
						if (delright) {
							uni.showModal({
								title: '提示',
								content: '是否确认删除收款单：' + item.F_BillID,
								success(rrr) {
									if (rrr.confirm) {
										uni.showLoading({
											title: '删除中...'
