	...
	<view>
		<view class="card">
			...
			<view class="rowBtn">
				...
				<u-button type="error" :plain="true" class="cpBtn" size="mini" @click="deleteCgthFun(item, index)">
					删除</u-button>
			</view>
		</view>
	</view>
	...
	export default {
		...
		methods: {
							...
							.then(res => {
								...
								if (this.item.F_BillID == '') {
									if (this.newBill == true) {
										...
										uni.navigateTo({
											url: './cgthselProduct?dykh=' + this.item.dykh +
												'&&clientcode=' + this.item.clientcode,
					...
					.then(res => {
						...
						if (saveright == true) {
							...
							let reqObj = {
								...
								djlx: 'CT',
								actdate: new Date(this.item.actdate).setHours(new Date(this.item.actdate)
									.getHours() + 8),
								enddate: new Date(this.item.enddate).setHours(new Date(this.item.enddate)
									.getHours() + 8),
							}
							uni.showLoading({
								title: '正在保存单据',
			...
			gotoDetailFun: function() {
				...
				uni.$cgthInfo = this.item;
				uni.navigateTo({
					url: './cgthDetail?index=' + this.index
				})
			},
			...
			// 删除订单
			deleteCgthFun: function(item, index) {
				var that = this;
				let delright = false;
				//确认权限
				let reqObjright = {
