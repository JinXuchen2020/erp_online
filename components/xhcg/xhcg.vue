	...
	<view>
		<view class="card">
			<view class="topRow1" @click="gotoDetailFun">
				...
				<view v-if="item.kfqrt">
					<text class="colorGray">首样检验报告：</text>
					<u-checkbox v-model="item.kfqrt"/>
				</view>
			...
			<view class="rowBtn">
				...
				<u-button type="error" :plain="true" class="cpBtn" size="mini"
					@click="deleteXhcgFun(item, index)">
					删除</u-button>
			</view>	
		</view>
	</view>
	...
	export default {
		...
		methods: {
								...
								.then(res => {
									...
									if (this.item.F_BillID == '') {
										if (this.newBill == true) {
											...
											uni.navigateTo({
												url: './xhcgselProduct?dykh=' + this.item.dykh+'&&clientcode='+this.item.clientcode,
			...
			gotoDetailFun: function() {
				...
				uni.$xhcgInfo = this.item;
				uni.navigateTo({
					url: './xhcgDetail?index=' + this.index
				})
			},
			...
			// 删除订单
			deleteXhcgFun: function(item, index) {
				var that = this;
				let delright = false;
				//确认权限
				let reqObjright = {
