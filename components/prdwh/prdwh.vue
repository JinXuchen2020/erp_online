<template>
	<view>
		<view class="card">
			...
			<view class="rowBtn">
				<u-button v-if='(pagetype === "入库详情")&&(checkbill !== true)' type="primary" :plain="true" class="cpBtn"
				...
				<u-button type="error" :plain="true" class="cpBtn" size="mini" @click="deletePrdwhFun(item, index)">
					删除</u-button>
			</view>
			<!--搜索弹窗-->
	...
	export default {
		...
		methods: {
					...
					.then(res => {
						...
						if (this.item.F_BillID == '') {
							if (this.newBill == true) {
								...
								uni.navigateTo({
									url: './prdwhselProduct?ckmc=' + this.item.ckmc,
			...
			gotoDetailFun: function() {
				...
				uni.$prdwhInfo = this.item;
				uni.navigateTo({
					url: './prdwhDetail?index=' + this.index
				})
			},
			...
			// 删除入库
			deletePrdwhFun: function(item, index) {
				var that = this;
				let delright = false;
				//确认权限
				let reqObjright = {
				...
				testrightApi(reqDataright)
					.then(res => {
						...
						if (delright) {
							uni.showModal({
								title: '提示',
								content: '是否确认删除入库：' + item.F_BillID,
								success(rrr) {
									if (rrr.confirm) {
										uni.showLoading({
											title: '删除中...'
