	...
	<view>
		<view class="card">
			...
			<view class="rowBtn">
				...
				<u-button v-if='(pagetype === "发货详情")&&(item.F_BillID)' type="warning" :plain="true" class="cpBtn"
					size="mini" @click="selprintfileFun">
					打印
				</u-button>
				<u-button type="error" :plain="true" class="cpBtn" size="mini"
					@click="deleteDelivergdsFun(item, index)">
					删除</u-button>
			</view>
		</view>
	</view>
		...
		methods: {
			...
			selproductFun: function(e) {
				...
				if (!this.item.clientcode) {
					...
					console.log('clientcode', !this.item.clientcode)
					let reqObj = {
						model: 'frmSellFh',
						usercode: uni.$userInfo.F_ID,
					}
						...
						.then(res => {
							...
							if (this.item.F_BillID == '') {
								if (this.newBill == true) {
									...
									uni.navigateTo({
										url: './delivergdsselProduct?clientcode=' + this.item.clientcode,
							...
							delivergdsApi(reqData)
								.then(res => {
									...
									if (this.item.F_BillID == '') {
										...
										uni.$addDelivergds = res.data.rows[0];
			...
			gotoDetailFun: function() {
				...
				uni.$delivergdsInfo = this.item;
				uni.navigateTo({
					url: './delivergdsDetail?index=' + this.index
				})
			},
			...
			// 删除发货
			deleteDelivergdsFun: function(item, index) {
				var that = this;
				let delright = false;
				//确认权限
				let reqObjright = {
				...
				testrightApi(reqDataright)
					.then(res => {
						...
						if (delright) {
							uni.showModal({
								title: '提示',
								content: '是否确认删除发货：' + item.F_BillID,
								success(rrr) {
									if (rrr.confirm) {
										uni.showLoading({
											title: '删除中...'
										...
										delivergdsApi(reqData)
											.then(res => {
												...
												uni.$emit('deleteDelivergdsFun', {
													index,
												})

												if (that.pagetype == "发货详情") {
													setTimeout(() => {
														uni.navigateBack()
													}, 666)
												}
