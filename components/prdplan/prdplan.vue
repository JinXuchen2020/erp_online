		...
		<view class="card">
			<view class="topRow1" @click="gotoDetailFun">
				...
				<view v-if="item.actdate">
					<text class="colorGray">计划日期：</text>
			...
			<view class="rowBtn">
				<u-button v-if='(pagetype === "计划详情")&&(checkbill !== true)' type="primary" :plain="true" class="cpBtn"
				...
				<u-button v-if='(pagetype === "计划详情")&&(item.F_BillID)' type="warning" :plain="true" class="cpBtn"
					size="mini" @click="selprintfileFun">
					打印
				</u-button>
				<u-button type="error" :plain="true" class="cpBtn" size="mini" @click="deletePrdplanFun(item, index)">
					删除</u-button>
			</view>
			<!--搜索弹窗-->
			<u-popup v-model="searchShow" mode="center" width="666rpx" border-radius="14" :closeable="false">
				<view class="searchBox">
					<view class="searchTitle">输入计划信息</view>
					<view class="dateBox">
						计划日期
	...
	export default {
		...
		methods: {
					...
					.then(res => {
						...
						if (this.item.F_BillID == '') {
							if (this.newBill == true) {
								...
								uni.navigateTo({
									url: './prdplanselProduct?ckmc=' + this.item.ckmc,
							...
							prdplanApi(reqData)
								.then(res => {
									...
									if (this.item.F_BillID == '') {
										...
										uni.$addPrdplan = res.data.rows[0];
			...
			gotoDetailFun: function() {
				...
				uni.$prdplanInfo = this.item;
				uni.navigateTo({
					url: './prdplanDetail?index=' + this.index
				})
			},
			...
			// 删除计划
			deletePrdplanFun: function(item, index) {
				var that = this;
				let delright = false;
				//确认权限
				let reqObjright = {
