		...
		<view class="card">
			<view class="topRow1" @click="gotoDetailFun">
				...
				<view v-if="item.clientname">
					<text class="colorGray">供方名称：</text>
					<text>{{item.clientname}}</text>
				</view>
				<view v-if="item.ckmc">
					<text class="colorGray">退货仓库：</text>
					<text>{{item.ckmc}}</text>
				</view>
			...
			<view class="rowBtn">
				...
				<u-button type="error" :plain="true" class="cpBtn" size="mini" @click="deleteOtsrtFun(item, index)">
					删除</u-button>
			</view>
			<!--搜索弹窗-->
			<u-popup v-model="searchShow" mode="center" width="666rpx" border-radius="14" :closeable="false">
				<view class="searchBox">
					...
					<view class="infoBox">
						<u-field v-model="item.clientname" :label="'委外厂家'" :placeholder="'请选择委外厂家'" clear-size="40"
					...
					<view class="infoBox">
						<u-field v-model="item.ckmc" :label="'退货仓库'" :placeholder="'请选择退货仓库'" clear-size="40"
							class="clientBox"></u-field>
						<u-button type="warning" class="searchBtn" :ripple="true" ripple-bg-color="#909399"
							:plain="true" size="medium" @click="selckmcFun(item.ckmc)">选择</u-button>
					</view>	
	...
	export default {
		...
		methods: {
						...
						.then(res => {
							...
							if (this.item.F_BillID == '') {
								if (this.newBill == true) {
									...
									uni.navigateTo({
										url: './otsrtselProduct?clientcode=' + this.item.clientcode+'&&ckmc='+ this.item.ckmc,
			...
			gotoDetailFun: function() {
				...
				uni.$otsrtInfo = this.item;
				uni.navigateTo({
					url: './otsrtDetail?index=' + this.index
				})
			},
			...
			// 删除退货
			deleteOtsrtFun: function(item, index) {
				var that = this;
				let delright = false;
				//确认权限
				let reqObjright = {
