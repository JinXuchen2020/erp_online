		...
		<view class="card">
			<view class="topRow1" @click="gotoDetailFun">
				...
				<view v-if="item.actdate">
					<text class="colorGray">出库日期：</text>
					<text>{{item.actdate}}</text>
				</view>

				<view v-if="item.zd">
					<text class="colorGray">出库人员：</text>
					<text>{{item.zd}}</text>
				</view>
				<view v-if="item.account">
					<text class="colorGray">出库金额：</text>
			...
			<view class="rowBtn">
				<u-button v-if='(pagetype === "出库详情")&&(checkbill !== true)' type="primary" :plain="true" class="cpBtn"
				...
				<u-button v-if='(pagetype === "出库详情")&&(item.F_BillID)' type="warning" :plain="true" class="cpBtn"
					size="mini" @click="selprintfileFun">
					打印
				</u-button>
				<u-button type="error" :plain="true" class="cpBtn" size="mini" @click="deleteOutboundFun(item, index)">
					删除</u-button>
			</view>
			<!--搜索弹窗-->
			<u-popup v-model="searchShow" mode="center" width="666rpx" border-radius="14" :closeable="false">
				<view class="searchBox">
					<view class="searchTitle">输入出库信息</view>
					<view class="dateBox">
						出库日期
					...
					<view class="infoBox">
						<u-field v-model="item.ckmc" :label="'仓库名称'" :placeholder="'请选择出库仓库'" clear-size="40"
					...
					<u-field v-model="item.bz" :label="'出库备注'" :placeholder="'请输入出库备注'" clear-size="40"></u-field>
	...
	export default {
		...
		methods: {
						...
						.then(res => {
							...
							if (this.item.F_BillID == '') {
								if (this.newBill == true) {
									...
									uni.navigateTo({
										url: './outboundselProduct?ckmc=' + this.item.ckmc,
							...
							outboundApi(reqData)
								.then(res => {
									...
									if (this.item.F_BillID == '') {
										...
										uni.$addOutbound = res.data.rows[0];
			...
			gotoDetailFun: function() {
				...
				uni.$outboundInfo = this.item;
				uni.navigateTo({
					url: './outboundDetail?index=' + this.index
				})
			},
			...
			// 删除出库
			deleteOutboundFun: function(item, index) {
				var that = this;
				let delright = false;
				//确认权限
				let reqObjright = {
				...
				testrightApi(reqDataright)
					.then(res => {
						...
						if (delright) {
							uni.showModal({
								title: '提示',
								content: '是否确认删除出库单：' + item.F_BillID,
								success(rrr) {
									if (rrr.confirm) {
										uni.showLoading({
											title: '删除中...'
										...
										outboundApi(reqData)
											.then(res => {
												...
												uni.$emit('deleteOutboundFun', {
													index,
												})

												if (that.pagetype == "出库详情") {
													setTimeout(() => {
														uni.navigateBack()
													}, 666)
												}
