		...
		<view class="card">
			...
			<view class="rowBtn">
				...
				<u-button type="error" :plain="true" class="cpBtn" size="mini"
					@click="deleteSalesorderFun(item, index)">
					删除</u-button>
			</view>	
		</view>
	</view>
	...
	export default {
		...
		methods: {
							...
							salesorderApi(reqData)
								.then(res => {
									...
									if (this.item.F_BillID == '') {
										...
										uni.$addSalesorder = res.data.rows[0];
									} 
			...
			gotoDetailFun: function() {
				...
				uni.$salesorderInfo = this.item;
				uni.navigateTo({
					url: './salesorderDetail?index=' + this.index
				})
			},
			...
			// 删除订单
			deleteSalesorderFun: function(item, index) {
				var that = this;
				let delright = false;
				//确认权限
				let reqObjright = {
