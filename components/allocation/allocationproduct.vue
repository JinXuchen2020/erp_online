					...
					<view v-else class="myCard">
						...
						<view class="myCard3">
							...
							<view class="infoBox">
								...
								<view class="cardRow">
									<view>调拨数量：</view>
									<view><text class="redColor">{{item.amount}}</text></view>
								</view>
								<view class="cardRow">
									<view>调拨单价：</view>
								...
								<view class="cardRow">
									<view>调拨金额：</view>
									<view><text class="redColor">{{item.total}}</text></view>
								</view>
								<view class="cardRow">
									<view>特别备注：</view>
									<view>{{item.bz}}</view>
	...
	export default {
		...
		methods: {
			...
			updateItemFun: function() {
				...
				this.item.amount=this.searchValue1;
				this.item.price=this.searchValue2;
				this.item.total=(+this.searchValue1)*(+this.searchValue2);
				...
				uni.showModal({
					...
					success(rrr) {
						if (rrr.confirm) {
							...
							let reqData = {
								action: 'delallocationdetailById',
							...
							allocationApi(reqData)
								.then(res => {
									...
									uni.$emit('delallocationdetailById', {
										index: that.index
									})
