		...
		<view class="card">
			<view class="topRow1" @click="gotoDetailFun">
				...
				<view v-if="item.actdate">
					<text class="colorGray">调拨日期：</text>
					<text>{{item.actdate}}</text>
				</view>

				<view v-if="item.zd">
					<text class="colorGray">调拨人员：</text>
					<text>{{item.zd}}</text>
				</view>
				<view v-if="item.ckmc">
					<text class="colorGray">调出仓库：</text>
					<text>{{item.ckmc}}</text>
				</view>
				<view v-if="item.ckmc1">
					<text class="colorGray">调入仓库：</text>
					<text>{{item.ckmc1}}</text>
				</view>
				<view v-if="item.account">
					<text class="colorGray">调拨金额：</text>
			...
			<view class="rowBtn">
				<u-button v-if='(pagetype === "调拨详情")&&(checkbill !== true)' type="primary" :plain="true" class="cpBtn"
					size="mini" @click="testrightFun">选仓库
				</u-button>
				<u-button v-if='(pagetype === "调拨详情")&&(checkbill !== true)' type="primary" :plain="true" class="cpBtn"
				...
				<u-button v-if='(pagetype === "调拨详情")&&(item.F_BillID)' type="warning" :plain="true" class="cpBtn"
					size="mini" @click="selprintfileFun">
					打印
				</u-button>
				<u-button type="error" :plain="true" class="cpBtn" size="mini" @click="deleteAllocationFun(item, index)">
					删除</u-button>
			</view>
			<!--搜索弹窗-->
			<u-popup v-model="searchShow" mode="center" width="666rpx" border-radius="14" :closeable="false">
				<view class="searchBox">
					<view class="searchTitle">输入调拨信息</view>
					<view class="dateBox">
						调拨日期
					...
					<view class="infoBox">
						<u-field v-model="item.ckmc" :label="'调出名称'" :placeholder="'请选择调出仓库'" clear-size="40"
							class="clientBox"></u-field>
						<u-button type="warning" class="searchBtn" :ripple="true" ripple-bg-color="#909399"
							:plain="true" size="medium" @click="selckmcFun(item.clientcode)">选择</u-button>
					</view>
					<view class="infoBox">
						<u-field v-model="item.ckmc1" :label="'调入仓库'" :placeholder="'请选择调入仓库'" clear-size="40"
							class="clientBox"></u-field>
						<u-button type="warning" class="searchBtn" :ripple="true" ripple-bg-color="#909399"
							:plain="true" size="medium" @click="selckmcFun1(item.clientcode)">选择</u-button>
					</view>
					<u-field v-model="item.bz" :label="'调拨备注'" :placeholder="'请输入调拨备注'" clear-size="40"></u-field>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 选择仓库名称函数
			selckmcFun: function(e) {
				uni.navigateTo({
					url: './allocationselCkmc'
				})
			},
			// 选择仓库名称函数
			selckmcFun1: function(e) {
				uni.navigateTo({
					url: './allocationselCkmc1'
				})
			},
			// 选择打印文件
						...
						.then(res => {
							...
							if (this.item.F_BillID == '') {
								if (this.newBill == true) {
									...
									uni.navigateTo({
										url: './allocationselProduct?ckmc=' + this.item.ckmc,
					...
					.then(res => {
						...
						if (saveright == true) {
							let reqObj = {
								...
								ckmc: this.item.ckmc,
								ckmc1: this.item.ckmc1,
								bz: this.item.bz,
								ckdb_content: this.itemList,
							}
							uni.showLoading({
								title: '正在保存单据',
								mask: true,
							...
							allocationApi(reqData)
								.then(res => {
									...
									if (this.item.F_BillID == '') {
										...
										uni.$addAllocation = res.data.rows[0];
			...
			gotoDetailFun: function() {
				...
				uni.$allocationInfo = this.item;
				uni.navigateTo({
					url: './allocationDetail?index=' + this.index
				})
			},
			...
			// 删除调拨
			deleteAllocationFun: function(item, index) {
				var that = this;
				let delright = false;
				//确认权限
				let reqObjright = {
				...
				testrightApi(reqDataright)
					.then(res => {
						...
						if (delright) {
							uni.showModal({
								title: '提示',
								content: '是否确认删除调拨单：' + item.F_BillID,
								success(rrr) {
									if (rrr.confirm) {
										uni.showLoading({
											title: '删除中...'
										...
										allocationApi(reqData)
											.then(res => {
												...
												uni.$emit('deleteAllocationFun', {
													index,
												})

												if (that.pagetype == "调拨详情") {
													setTimeout(() => {
														uni.navigateBack()
													}, 666)
												}
