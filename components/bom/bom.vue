		...
		<view class="card">
			<view class="topRow1" @click="gotoDetailFun">
				...
				<view v-if="item.actdate">
					<text class="colorGray">登记日期：</text>
					<text>{{item.actdate}}</text>
				</view>			
				<view v-if="item.pname">
					<text class="colorGray">成品名称：</text>
					<text>{{item.pname}}</text>
				</view>	
				<view v-if="item.pspec">
					<text class="colorGray">成品规格：</text>
					<text>{{item.pspec}}</text>
				</view>	
				<view v-if="item.pcolor">
					<text class="colorGray">成品颜色：</text>
					<text>{{item.pcolor}}</text>
				</view>	
				<view v-if="item.model">
					<text class="colorGray">成品型号：</text>
					<text>{{item.model}}</text>
				</view>	
				<view v-if="item.zd">
					<text class="colorGray">登记人员：</text>
					<text>{{item.zd}}</text>
				</view>
				<view v-if="item.account">
					<text class="colorGray">BOM成本：</text>
			...
			<view class="rowBtn">	
				<u-button v-if='(pagetype === "清单详情")&&(checkbill !== true)' type="primary" :plain="true" class="cpBtn"
					size="mini" @click="testrightFun">选成品
				</u-button>
				<u-button v-if='(pagetype === "清单详情")&&(checkbill !== true)' type="primary" :plain="true" class="cpBtn"
					size="mini" @click="selproductFun">选配件
				</u-button>
				<u-button v-if='(pagetype === "清单详情")&&(checkbill !== true)' type="primary" :plain="true" class="cpBtn"
			...
			<u-popup v-model="searchShow" mode="center" width="666rpx" border-radius="14" :closeable="false">
				<view class="searchBox">
					...
					<view class="infoBox">
						<u-field v-model="item.pcode" :label="'成品编号'" :placeholder="'请选择成品'" clear-size="40"
							class="clientBox"></u-field>
						<u-button type="warning" class="searchBtn" :ripple="true" ripple-bg-color="#909399"
							:plain="true" size="medium" @click="selpcodeFun(item.clientcode)">选择</u-button>
					</view>
					<view class="infoBox">
						<u-field v-model="item.pname" :label="'成品名称'" clear-size="40"
							class="clientBox"></u-field>						
					</view>
					<view class="infoBox">
						<u-field v-model="item.pspec" :label="'成品规格'" clear-size="40"
							class="clientBox"></u-field>						
					</view>
					<view class="infoBox">
						<u-field v-model="item.pcolor" :label="'成品颜色'" clear-size="40"
							class="clientBox"></u-field>						
					</view>
					<view class="infoBox">
						<u-field v-model="item.model" :label="'成品型号'" clear-size="40"
							class="clientBox"></u-field>						
					</view>
					<u-field v-model="item.bz" :label="'清单备注'" :placeholder="'请输入清单备注'" clear-size="40"></u-field>
	...
	export default {
		...
		methods: {
					...
					.then(res => {
						...
						if (this.item.F_BillID == '') {
							if (this.newBill == true) {
								...
								uni.navigateTo({
									url: './bomselProduct?ckmc='+this.item.ckmc,
					...
					.then(res => {
						...
						if (saveright == true) {
							let reqObj = {
								...
								pcode: this.item.pcode,
								pname: this.item.pname,
								pspec: this.item.pspec,
								pcolor: this.item.pcolor,
								account: this.item.account,
								actdate: this.item.actdate,
								model: this.item.model,
								bz: this.item.bz,
								list_content: this.itemList,
							}
							uni.showLoading({
								title: '正在保存单据',
								mask: true,
			...
			gotoDetailFun: function() {
				...
				uni.$bomInfo = this.item;
				uni.navigateTo({
					url: './bomDetail?index=' + this.index
				})
			},
			...
			// 删除清单
			deleteBomFun: function(item, index) {
				var that = this;
				let delright = false;
				//确认权限
				let reqObjright = {
				...
				testrightApi(reqDataright)
					.then(res => {
						...
						if (delright) {
							uni.showModal({
								title: '提示',
								content: '是否确认删除清单：' + item.F_BillID,
								success(rrr) {
									if (rrr.confirm) {
										uni.showLoading({
											title: '删除中...'
										...
										bomApi(reqData)
											.then(res => {
												...
												uni.$emit('deleteBomFun', {
													index,
												})

												if (that.pagetype == "清单详情") {
													setTimeout(() => {
														uni.navigateBack()
													}, 666)
												}
