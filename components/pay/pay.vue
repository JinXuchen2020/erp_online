	...
	<view>
		<view class="card">
			<view class="topRow1" @click="gotoDetailFun">
				...
				<view class="infoBox">
					<view class="name">
						<text>{{item.F_SupplierName || '暂无'}}</text>
					</view>
				...
				<view v-if="item.F_SupplierID">
					<text class="colorGray">供方编号：</text>
					<text>{{item.F_SupplierID}}</text>
				</view>
				...
				<view v-if="item.F_Kind">
					<text class="colorGray">付款类型：</text>
					<text>{{item.F_Kind}}</text>
				</view>
				<view v-if="item.f_bank">
					<text class="colorGray">付款账户：</text>
					<text>{{item.f_bank}}</text>
				</view>
				<view v-if="item.F_Money">
					<text class="colorGray">应付金额：</text>
					<text>{{item.F_Money}}</text>
				</view>
				<view v-if="item.F_Tax">
					<text class="colorGray">税额：</text>
					<text>{{item.F_Tax}}</text>
				</view>
				<view v-if="item.F_Total">
					<text class="colorGray">税价合计：</text>
					<text>{{item.F_Total}}</text>
				</view>
				<view v-if="item.F_DisMoney">
					<text class="colorGray">扣减金额：</text>
					<text>{{item.F_DisMoney}}</text>
				</view>
				<view v-if="item.F_dzje">
					<text class="colorGray">实付金额：</text>
					<text>{{item.F_dzje}}</text>
				</view>
				...
				<view v-if="item.F_PayType">
					<text class="colorGray">付款方式：</text>
					<text>{{item.F_PayType}}</text>
				</view>
				<view v-if="item.dykh">
			...
			<view class="rowBtn">
				...
				<u-button v-if='(pagetype === "付款详情")&&(checkbill !== true)' type="primary" :plain="true" class="cpBtn"
					size="mini" @click="selproductFun">调单
				</u-button>
				<u-button v-if='(pagetype === "付款详情")&&(checkbill !== true)' type="primary" :plain="true" class="cpBtn"
				...
				<u-button type="error" :plain="true" class="cpBtn" size="mini" @click="deletePayFun(item, index)">
					删除</u-button>
			</view>
			<!--搜索弹窗-->
			<u-popup v-model="searchShow" mode="center" width="666rpx" border-radius="14" :closeable="false">
				<view class="searchBox">
					...
					<view class="infoBox">
						<u-field v-model="item.F_ClientName" :label="'客户名称'" :placeholder="'请输入客户'" clear-size="40"
							class="clientBox"></u-field>
						<u-button type="warning" class="searchBtn" :ripple="true" ripple-bg-color="#909399"
							:plain="true" size="medium" @click="selclientFun(item)">选择客户</u-button>


					</view>
					<view class="dateBox">
						付款日期
						<picker mode="date" :value="item.F_Date" start="1999-02-15" end="2100-02-15"
							@change="bindsDateChange">
							<text class="dateText">{{item.F_Date}}</text>
						</picker>
					</view>


					<view class="infoBox">
						<u-field v-model="item.f_bank" :label="'付款账户'" :placeholder="'请选择付款账户'" clear-size="40"
							class="clientBox"></u-field>
						<u-button type="warning" class="searchBtn" :ripple="true" ripple-bg-color="#909399"
							:plain="true" size="medium" @click="selbankFun(item.F_SupplierID)">选择</u-button>


					</view>
					<u-field v-model="item.F_PayType" :label="'付款方式'" :placeholder="'请选择付款方式'"
						clear-size="40"></u-field>
					<u-field v-model="item.bz" :label="'付款备注'" :placeholder="'请输入付款备注'" clear-size="40"></u-field>
	...
	export default {
		...
		methods: {
			...
			selproductFun: function(e) {
				...
				testrightApi(reqData)
					.then(res => {
						...
						that.printBill = res.data.printBill
						that.checkBill = res.data.checkBill
						console.log('得到付款单号：', this.item.F_BillID)
						if (this.item.F_BillID == '') {
							if (this.newBill == true) {
								console.log('clientId', this.item.F_SupplierID)
								if (this.item.F_SupplierID == '') {
									uni.showModal({
										title: '提示',
										content: '请选择付款供方',
										showCancel: false
									...
									uni.setStorageSync("paydetail", this.itemList)
									console.log('新增单据传递过去的itemlist', this.itemList)
									uni.navigateTo({
										url: './payselProduct?F_SupplierID=' + this.item.F_SupplierID,
					...
					.then(res => {
						...
						if (saveright == true) {
							let reqObj = {
								...
								F_Tax:this.item.F_Tax,
								F_Total:this.item.F_Total,
								F_dzje:this.item.F_dzje,
								currency:this.item.currency,
								hl:this.item.hl,
								rmb:this.item.rmb,
								t_paydetail: this.itemList,
							}
							uni.showLoading({
								title: '正在保存单据',
								mask: true,
			...
			gotoDetailFun: function() {
				...
				uni.$payInfo = this.item;
				uni.navigateTo({
					url: './payDetail?index=' + this.index
				})
			},
			...
			// 删除付款单
			deletePayFun: function(item, index) {
				var that = this;
				let delright = false;
				//确认权限
				let reqObjright = {
