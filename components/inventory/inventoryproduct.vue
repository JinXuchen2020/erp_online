					...
					<view v-else class="myCard">
						...
						<view class="myCard3">
							...
							<view class="infoBox">
								...
								<view class="cardRow">
									<view>盘亏数量：</view>
									<view><text class="redColor">{{item.pksl}}</text></view>
								</view>
								<view class="cardRow">
									<view>特别备注：</view>
									<view>{{item.bz}}</view>
	...
	export default {
		...
		methods: {
			...
			updateItemFun: function() {
				...
				this.item.kcsl=this.searchValue2;
				this.item.pksl=(+this.searchValue1)-(+this.searchValue2);
						...
						if (rrr.confirm) {
							...
							inventoryApi(reqData)
								.then(res => {
									...
									uni.$emit('delinventorydetailById', {
										index: that.index
									})
