		...
		<view class="card">
			<view class="topRow1" @click="gotoDetailFun">
				...
				<view v-if="item.actdate">
					<text class="colorGray">盘点日期：</text>
					<text>{{item.actdate}}</text>
				</view>			
				
				<view v-if="item.zd">
					<text class="colorGray">盘点录入：</text>
					<text>{{item.zd}}</text>
				</view>
				<view v-if="item.bz">
			...
			<view class="rowBtn">	
				<u-button v-if='(pagetype === "盘点详情")&&(checkbill !== true)' type="primary" :plain="true" class="cpBtn"
			...
			<u-popup v-model="searchShow" mode="center" width="666rpx" border-radius="14" :closeable="false">
				<view class="searchBox">
					...
					<view class="infoBox">
						<u-field v-model="item.ckmc" :label="'仓库名称'" :placeholder="'请选择盘点仓库'" clear-size="40"
					...
					<u-field v-model="item.bz" :label="'盘点备注'" :placeholder="'请输入盘点备注'" clear-size="40"></u-field>
	...
	export default {
		...
		methods: {
					...
					.then(res => {
						...
						if (this.item.F_BillID == '') {
							if (this.newBill == true) {
								...
								uni.navigateTo({
									url: './inventoryselProduct?ckmc='+this.item.ckmc,
							...
							inventoryApi(reqData)
								.then(res => {
									...
									if (this.item.F_BillID == '') {
										...
										uni.$addInventory = res.data.rows[0];
			...
			gotoDetailFun: function() {
				...
				uni.$inventoryInfo = this.item;
				uni.navigateTo({
					url: './inventoryDetail?index=' + this.index
				})
			},
			...
			// 删除盘点
			deleteInventoryFun: function(item, index) {
				var that = this;
				let delright = false;
				//确认权限
				let reqObjright = {
				...
				testrightApi(reqDataright)
					.then(res => {
						...
						if (delright) {
							uni.showModal({
								title: '提示',
								content: '是否确认删除盘点单：' + item.F_BillID,
								success(rrr) {
									if (rrr.confirm) {
										uni.showLoading({
											title: '删除中...'
										...
										inventoryApi(reqData)
											.then(res => {
												...
												uni.$emit('deleteInventoryFun', {
													index,
												})

												if (that.pagetype == "盘点详情") {
													setTimeout(() => {
														uni.navigateBack()
													}, 666)
												}
